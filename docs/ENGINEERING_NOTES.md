...
- Computer assistant: deterministic status/command index plus read-only file organization, disk cleanup, app install troubleshooting, and peripheral setup planning. These commands may sample top-level folder/drive state where safe, but do not move/delete files, run installers, change devices, or change Windows settings.
...
Ability-index maintenance is now a product rule. Each new feature should update the deterministic command surfaces (`show coding skill command index`, `show computer assistant commands` or a more specific index), relevant Web Helper/VS Companion buttons, user documentation, engineering notes, and tests. Natural ability questions such as "what can you do" and "can you tell me about your abilities" should route to deterministic status/index output instead of relying on the model to guess current app powers.
