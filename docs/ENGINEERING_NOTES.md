...
Ali source access is planner-driven and evidence-oriented. The app asks `ModelSourceQueryPlanner` whether a user turn needs sources, then routes internet lookups through `TavilyFirecrawlSourceRetriever` and local document lookups through `LocalVectorLibraryRetriever`. The model decides source need and routing from the user turn plus recent context; the orchestration layer does not keyword-match user English to pick internet tools.

Internet settings live in `%LOCALAPPDATA%\Ali\BootstrapData\Sources\internet_backends.json` and can also resolve Tavily/Firecrawl keys from environment variables. Tavily is the primary search provider. Firecrawl provides fallback search and page extraction. The WPF Settings window exposes the enabled state and API keys; advanced endpoint and extraction settings remain in the JSON file.

When source excerpts are present, `SourcePromptFormatter` wraps them as untrusted evidence and tells the model to cite bracketed source numbers. The app owns the visible `Sources checked:` appendix and strips any model-generated duplicate. When the planner asked for sources but the backend returns no excerpts, Ali reports the source failure instead of inventing current facts.
...
Local document references are handled by the Local Library index under `%LOCALAPPDATA%\Ali\BootstrapData\Sources`. That path is separate from Tavily/Firecrawl internet lookup and should remain limited to user-approved local documents.
