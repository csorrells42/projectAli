...
Analyzer-config document deltas can cause Roslyn to derive a new `SyntaxTreeOptionsProvider` even when no independent compilation option changed. CP7 recognizes only that derived difference: when an authenticated analyzer-config delta exists, it removes that provider from both options through the exact pinned Roslyn 5.6 surface and compares every remaining option. Analyzer-config bytes remain part of the durable delta and semantic fingerprint. Missing or changed pinned members, type mismatches, invocation failures, or any accompanying independent option mutation fail closed.
...
Windows also cannot rename a directory root while Ali keeps descendant handles open. Workstation file-tree and conversation work-memory publication therefore take a final authenticated no-follow snapshot with the complete closure held, release only descendant handles, keep the root and namespace-parent handles identity-bound, perform a native no-replace rename, and immediately reseal and verify the complete destination tree before success. A failed rename reseals the original path; a successful rename that cannot be resealed cannot be treated as safe rollback. A persistent mutation in that interval cannot become success and reconciles `Unknown`; an interruption leaving the exact authenticated poststate reconciles `Applied`. This is a fail-closed integrity/recovery boundary, not an OS-enforced sandbox: an owner-operated same-user process can race the interval and may be indistinguishable if it restores the exact authenticated path/content snapshot before reseal. A future low-privilege worker with private scratch storage and an OS-enforced write boundary is required to close that residual race.
...
18. Normalize only analyzer-config-derived `SyntaxTreeOptionsProvider` state through the exact pinned Roslyn surface while comparing every independent compilation option.
19. Perform source and directory publication with authenticated native no-replace renames, destination-name identity verification, and durable `Unknown` classification for any unresealed post-rename state.
20. Publish durable receipts only from flushed, identity-verified temporary files whose final names are revalidated after the namespace transition.
21. Permit legitimate multi-linked executables only under an explicit pinned system/provider root after authenticating every alias; reject any alias that leaves that root.
