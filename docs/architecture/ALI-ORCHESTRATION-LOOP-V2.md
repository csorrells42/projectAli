...
`AliExecutionBroker` enforces the boundary for every descriptor with `EffectKind=SourceMutation`. Roslyn, generic file write/replace, formatter, LSP, package, shell/process, Skill, and MCP mutations receive write access only to the staged workspace and manifest; canonical source is read-only to that operation. A tool that cannot honor the staging boundary is not callable as a source mutation. Builds and reads are excluded while the brief canonical publish lease is held, and direct MCP invocation follows the same rule.
