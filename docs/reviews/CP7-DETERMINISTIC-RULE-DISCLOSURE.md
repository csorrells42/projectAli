...
- A provider action that changes project identity, project/reference metadata, compilation metadata, parse metadata, metadata references, analyzer references, or an unrepresented document metadata-only field is rejected. Those changes require a separate explicit project-document transaction that CP7 does not silently synthesize.
