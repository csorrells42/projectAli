...
public sealed class ModelCodingActionPlanner(ILocalModelRuntime runtime) : ICodingActionPlanner
{
    ...
    private static string BuildPlannerInstruction(IReadOnlyList<ChatMessage> history)
    {
        var lines = new List<string>
        {
            ...
            "- Starter app requests: for simple console or WPF apps, use build this for me <goal>; the patch lane can preview starter code for console calculators, guessing games, todo/list/file-backed apps, and WPF hello/counter/calculator/greeting/todo/dashboard windows before owner apply.",
            ...
            "- WPF layout/object checklist: choose Window/UserControl boundaries, Grid/DockPanel/StackPanel layout, ContentControl regions, ItemsControl/DataGrid/TreeView lists, ResourceDictionary styles/templates, bindings, commands, validation, virtualization, and async UI state before patching complex windows.",
