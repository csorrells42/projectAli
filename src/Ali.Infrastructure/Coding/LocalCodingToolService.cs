    ...
    string? pdfWorkspaceRoot = null) : ILocalCodingTool
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = request.Action switch
        {
            ...
            CodingToolAction.ShowFeatureWorkContext => await ShowFeatureWorkContextAsync(request, cancellationToken).ConfigureAwait(false),
            ...
            CodingToolAction.ShowBuildThisFeature => await ShowBuildThisFeatureAsync(request, cancellationToken).ConfigureAwait(false),
            CodingToolAction.ShowRoslynEditPlannerV2 => await ShowRoslynEditPlannerV2Async(request, cancellationToken).ConfigureAwait(false),
            CodingToolAction.ShowMultiFilePatchSynthesisV2 => await ShowMultiFilePatchSynthesisV2Async(request, cancellationToken).ConfigureAwait(false),
            CodingToolAction.ShowPatternCopyPlan => await ShowPatternCopyPlanAsync(request, cancellationToken).ConfigureAwait(false),
            CodingToolAction.ShowBehaviorTestGeneratorV2 => await ShowBehaviorTestGeneratorV2Async(request, cancellationToken).ConfigureAwait(false),
            CodingToolAction.ShowImplementationSliceState => await ShowImplementationSliceStateAsync(request, cancellationToken).ConfigureAwait(false),
            CodingToolAction.ShowPostApplyRepairLoopV2 => await ShowPostApplyRepairLoopV2Async(request, cancellationToken).ConfigureAwait(false),
            CodingToolAction.ShowSemanticDiffSummary => await ShowSemanticDiffSummaryAsync(request, cancellationToken).ConfigureAwait(false),
            CodingToolAction.ShowMiniCodexScoreV3 => await ShowMiniCodexScoreV3Async(request, cancellationToken).ConfigureAwait(false),
            CodingToolAction.ShowConcretePatchAuthoring => await ShowConcretePatchAuthoringAsync(request, cancellationToken).ConfigureAwait(false),
            CodingToolAction.ShowPatchBodyGenerator => await ShowPatchBodyGeneratorAsync(request, cancellationToken).ConfigureAwait(false),
            CodingToolAction.ShowPatternCommandScaffolder => await ShowPatternCommandScaffolderAsync(request, cancellationToken).ConfigureAwait(false),
            CodingToolAction.ShowUiBundlePlanner => await ShowUiBundlePlannerAsync(request, cancellationToken).ConfigureAwait(false),
            CodingToolAction.ShowPatchConfidenceScore => await ShowPatchConfidenceScoreAsync(request, cancellationToken).ConfigureAwait(false),
            CodingToolAction.ShowSliceExecutorPreview => await ShowSliceExecutorPreviewAsync(request, cancellationToken).ConfigureAwait(false),
        ...
        CancellationToken cancellationToken)
    {
        ...
        lines.Add("Change impact preview:");
        lines.AddRange(BuildChangeImpactPreviewLines(goal));
        ...
        CancellationToken cancellationToken)
    {
        ...
        var prerequisiteBlocks = await BuildPacketCommandPrerequisiteBlocksAsync(_approvedPacket, item, cancellationToken).ConfigureAwait(false);
        if (prerequisiteBlocks.Count > 0)
        {
            var blockedLines = new List<string>
            {
                $"Packet prerequisite blocked item {item.Number}.",
            ...
            blockedLines.Add("- show coding receipts");
            return new CodingToolResult(
                true,
                false,
                string.Join(Environment.NewLine, blockedLines),
                "Packet command console",
                _approvedPacketPath);
        }
    ...
    private async Task<IReadOnlyList<string>> BuildPacketCommandPrerequisiteBlocksAsync(
        ApprovedRoadmapExecutionPacket packet,
        PacketCommandItem item,
        CancellationToken cancellationToken)
    {
        var command = item.Command.Trim();
        if (!PacketCommandNeedsConfirmation(command) || IsPacketPlanningStateCommand(command))
        {
            return [];
        }
        ...
        var gitStatus = await InspectGitWorkingTreeAsync(cancellationToken).ConfigureAwait(false);
        LoadPendingPatchPreviewIfNeeded();
        var latestDotNet = receipts.LastOrDefault(IsDotNetReceipt);
        var latestValidationFailed = latestDotNet is { Succeeded: false };
        var hasPrep = receipts.Any(IsPacketPrepReceipt);
        var hasExecution = receipts.Any(IsPacketExecutionReceipt);
        var hasValidation = receipts.Any(IsPacketValidationReceipt) && !latestValidationFailed;
        var blocks = new List<string>();

        if (IsPacketExecutionMutation(command) && !hasPrep)
        {
            blocks.Add("Run or show a packet prep command first so the packet has current context.");
        }

        if (latestValidationFailed && !IsPatchOrDiagnosticCommand(command))
        {
            blocks.Add("Latest dotnet-style validation failed; diagnose or patch that failure before running non-fix commands.");
        }

        if (IsPatchApplyCommand(command) && _lastPatchPreviewRequest is null)
        {
            blocks.Add("No pending patch preview exists; preview the exact patch before applying it.");
        }

        if (IsCloseoutMutation(command))
        {
            if (!hasExecution)
            {
                blocks.Add("No execution receipt exists for this packet yet.");
            }

            if (!hasValidation)
            {
                blocks.Add("No successful validation receipt exists for this packet yet.");
            }

            if (_lastPatchPreviewRequest is not null)
            {
                blocks.Add("A pending patch preview still exists; apply or discard it before closeout.");
            }
        }

        if (IsGitWriteCommand(command) && !gitStatus.Available)
        {
            blocks.Add("Git status is unavailable; inspect Git before running packet Git writes.");
        }

        if (IsGitCommitCommand(command) && !hasValidation)
        {
            blocks.Add("Git commit needs a successful build/test validation receipt in this packet.");
        }

        return blocks;
    }

    private static bool IsPacketPlanningStateCommand(string command) =>
        command.StartsWith("discard ", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("show ", StringComparison.OrdinalIgnoreCase);

    private static bool IsPacketExecutionMutation(string command) =>
        IsPatchApplyCommand(command)
        || command.StartsWith("confirm dotnet restore", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("confirm dotnet add", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("confirm dotnet run", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("confirm run project", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("create file", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("append file", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("append to file", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("replace in file", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("replace text in file", StringComparison.OrdinalIgnoreCase);

    private static bool IsPatchOrDiagnosticCommand(string command) =>
        command.Contains("diagnose", StringComparison.OrdinalIgnoreCase)
        || command.Contains("suggest patch", StringComparison.OrdinalIgnoreCase)
        || command.Contains("preview patch", StringComparison.OrdinalIgnoreCase)
        || IsPatchApplyCommand(command);

    private static bool IsPatchApplyCommand(string command) =>
        command.StartsWith("confirm apply last patch preview", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("apply last patch preview", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("apply patch preview", StringComparison.OrdinalIgnoreCase);

    private static bool IsCloseoutMutation(string command) =>
        command.StartsWith("mark roadmap step complete", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("advance roadmap step", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("finish roadmap", StringComparison.OrdinalIgnoreCase)
        || IsGitWriteCommand(command);

    private static bool IsGitWriteCommand(string command) =>
        command.StartsWith("confirm git add", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("confirm git commit", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("confirm git merge", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("confirm git pull", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("confirm git push", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("git add", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("git commit", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("git merge", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("git pull", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("git push", StringComparison.OrdinalIgnoreCase);

    private static bool IsGitCommitCommand(string command) =>
        command.StartsWith("confirm git commit", StringComparison.OrdinalIgnoreCase)
        || command.StartsWith("git commit", StringComparison.OrdinalIgnoreCase);
    ...
    private CodingToolResult ClassifyLastFailure()
    {
        ...
        lines.AddRange(BuildFailureTriageLines(_lastDotNetRequest.Action, result.Message));
        lines.Add("Next safe commands:");
    ...
    private static string ClassifyCodingTask(string goal)
    {
        if (MentionsAny(goal, "commit", "release notes", "patch notes", "ship", "deploy"))
        {
            return "release/commit";
        }
        if (MentionsAny(goal, "build", "compile", "error", "failure", "failed", "exception", "bug", "fix", "repair"))
        {
            return "bug fix";
        }
        if (MentionsAny(goal, "test", "coverage", "assert", "regression"))
        {
            return "test";
        }
        if (MentionsAny(goal, "refactor", "rename", "move", "extract", "split"))
        {
            return "refactor";
        }
        if (MentionsAny(goal, "add", "implement", "create", "feature", "button", "dashboard"))
        {
            return "feature";
        }
        if (MentionsAny(goal, "installer", "setup", "package", "shortcut"))
        {
            return "packaging";
        }
        if (MentionsAny(goal, "docs", "document", "readme", "explain"))
        {
            return "documentation";
        }

        return "diagnostics/planning";
    }

    private static string BuildCodingTaskRiskPreview(string goal)
    {
        var risks = new List<string>();
        if (MentionsAny(goal, "installer", "setup", "deploy", "package", "shortcut"))
        {
            risks.Add("installer/delivery");
        }
        if (MentionsAny(goal, "runtime", "ollama", "model", "openai", "internet", "source", "weather"))
        {
            risks.Add("runtime/data access");
        }
        if (MentionsAny(goal, "voice", "audio", "speaker", "tts"))
        {
            risks.Add("voice/audio");
        }
        if (MentionsAny(goal, "button", "dashboard", "window", "xaml", "screen"))
        {
            risks.Add("desktop UI");
        }
        if (MentionsAny(goal, "git", "commit", "merge", "push", "pull"))
        {
            risks.Add("git workflow");
        }

        return risks.Count == 0 ? "normal coding risk" : string.Join(", ", risks.Distinct(StringComparer.OrdinalIgnoreCase));
    }

    private static IReadOnlyList<string> BuildChangeImpactPreviewLines(string goal)
    {
        var lines = new List<string>();
        lines.Add($"- Task class: {ClassifyCodingTask(goal)}");
        lines.Add($"- Risk areas: {BuildCodingTaskRiskPreview(goal)}");
        if (MentionsAny(goal, "button", "dashboard", "xaml", "command"))
        {
            lines.Add("- Likely files: XAML dashboard, view-model command, parser/service route, tests.");
        }
        else if (MentionsAny(goal, "build", "compiler", "failure", "error"))
        {
            lines.Add("- Likely files: first diagnostic file, owning project, nearest focused test.");
        }
        else if (MentionsAny(goal, "installer", "setup", "deploy", "package"))
        {
            lines.Add("- Likely files: installer options/service/window, publish output, readiness tests.");
        }
        else
        {
            lines.Add("- Likely files: resolve by project intent, ownership map, and semantic search before editing.");
        }
    ...
    private ProjectIndexAwareness LoadProjectIndexAwareness()
    {
        ...
        try
        {
            ...
            return new ProjectIndexAwareness(
                ...
                ReadProjectStringArray(root, "projectIntent"),
                ReadProjectStringArray(root, "compressedRepoMap"));
        }
    ...
    private static void AddProjectIndexAwarenessLines(List<string> lines, ProjectIndexAwareness awareness, int limit)
    {
        ...
        lines.Add($"- Refresh automation: {FormatInlineList(BuildProjectIndexRefreshRows(new ProjectIndexStatus(awareness.Available, awareness.Status)).Take(3))}");
        ...
        lines.Add($"- Project intent: {FormatInlineList(awareness.ProjectIntent.Take(limit))}");
        lines.Add($"- Compressed repo map: {FormatInlineList(awareness.CompressedRepoMap.Take(limit))}");
    }
    ...
    private IReadOnlyList<string> BuildProjectIntentSignals(
        IReadOnlyList<string> files,
        IReadOnlyList<ProjectSummary> summaries,
        IReadOnlyList<string> stackSignals)
    {
        var relativeFiles = files.Select(RelativeToWorkspace).ToList();
        var appKinds = new List<string>();
        if (relativeFiles.Any(path => path.EndsWith(".xaml", StringComparison.OrdinalIgnoreCase))
            || summaries.Any(summary => summary.XamlFileCount > 0)
            || stackSignals.Any(signal => signal.Contains("WPF", StringComparison.OrdinalIgnoreCase)))
        {
            appKinds.Add("desktop app");
        }

        if (relativeFiles.Any(path => path.Contains("Controller", StringComparison.OrdinalIgnoreCase)
        ...
        {
            appKinds.Add("web/service surface");
        }

        if (summaries.Any(summary => summary.ProjectRole.Contains("test", StringComparison.OrdinalIgnoreCase))
            || relativeFiles.Any(path => path.StartsWith("tests/", StringComparison.OrdinalIgnoreCase)))
        {
            appKinds.Add("test suite");
        }

        if (relativeFiles.Any(path => path.Contains("Installer", StringComparison.OrdinalIgnoreCase)
            || path.Contains("Installation", StringComparison.OrdinalIgnoreCase)
            || path.Contains("Distributable", StringComparison.OrdinalIgnoreCase)))
        {
            appKinds.Add("installer/delivery package");
        }

        if (relativeFiles.Any(path => path.Contains("VisualStudio", StringComparison.OrdinalIgnoreCase)
            || path.EndsWith(".vsixmanifest", StringComparison.OrdinalIgnoreCase)))
        {
            appKinds.Add("IDE extension/bridge");
        }

        if (appKinds.Count == 0 && summaries.Count > 0)
        {
            appKinds.Add("library/support codebase");
        }

        var primaryWorkflow = appKinds.Contains("desktop app")
            ? "build WPF app, validate coding/maintenance dashboards, then package installer"
            : appKinds.Contains("web/service surface")
                ? "build service, run tests, validate endpoint behavior"
                : "inspect project target, run focused tests, then build";

        return
        [
            $"Detected repo shape: {FormatInlineList(appKinds.Distinct(StringComparer.OrdinalIgnoreCase))}",
            $"Primary workflow: {primaryWorkflow}",
            $"Delivery surface: {FormatDeliverySurface(relativeFiles)}",
            $"Validation personality: {FormatValidationPersonality(summaries, relativeFiles)}",
            "Intent rule: decide app shape before choosing edit, build, test, and release commands."
        ];
    }

    private static string FormatDeliverySurface(IReadOnlyList<string> relativeFiles)
    {
        var surfaces = new List<string>();
        if (relativeFiles.Any(path => path.Contains("Ali.App.Wpf", StringComparison.OrdinalIgnoreCase)))
        {
            surfaces.Add("WPF client");
        }
        if (relativeFiles.Any(path => path.Contains("Installer", StringComparison.OrdinalIgnoreCase)))
        {
        ...
        if (relativeFiles.Any(path => path.Contains("VisualStudio", StringComparison.OrdinalIgnoreCase)))
        {
            surfaces.Add("Visual Studio bridge");
        }
        if (relativeFiles.Any(path => path.Contains("WebHelper", StringComparison.OrdinalIgnoreCase)))
        {
            surfaces.Add("local web helper");
        }

        return FormatInlineList(surfaces.Distinct(StringComparer.OrdinalIgnoreCase));
    }

    private static string FormatValidationPersonality(IReadOnlyList<ProjectSummary> summaries, IReadOnlyList<string> relativeFiles)
    {
        var hasTests = summaries.Any(summary => summary.ProjectRole.Contains("test", StringComparison.OrdinalIgnoreCase))
            || relativeFiles.Any(path => path.StartsWith("tests/", StringComparison.OrdinalIgnoreCase));
        var hasInstaller = relativeFiles.Any(path => path.Contains("Installer", StringComparison.OrdinalIgnoreCase)
            || path.Contains("Installation", StringComparison.OrdinalIgnoreCase));
        var hasUi = relativeFiles.Any(path => path.EndsWith(".xaml", StringComparison.OrdinalIgnoreCase)
            || path.Contains("App.Wpf", StringComparison.OrdinalIgnoreCase));
        var cards = new List<string> { hasTests ? "targeted tests available" : "manual/build validation needed" };
        if (hasUi)
        {
            cards.Add("UI binding checks matter");
        }
        if (hasInstaller)
        {
            cards.Add("installer smoke checks matter");
        }

        return string.Join("; ", cards);
    }

    private IReadOnlyList<string> BuildCompressedRepoMap(
        IReadOnlyList<string> files,
        IReadOnlyList<ProjectSummary> summaries,
        IReadOnlyList<ProjectIndexFileRole> fileRoles,
        IReadOnlyList<ProjectIndexFeatureArea> featureAreas,
        IReadOnlyList<string> projectIntent,
        IReadOnlyList<string> buildCommands,
        IReadOnlyList<string> testCommands)
    {
        var entryFiles = files.Where(IsLikelyEntryPoint).Select(RelativeToWorkspace).Take(5).ToList();
        var dashboardFiles = files
            .Select(RelativeToWorkspace)
            .Where(path => path.Contains("Dashboard", StringComparison.OrdinalIgnoreCase)
        ...
        return
        [
            $"Intent: {FormatInlineList(projectIntent.Take(3))}",
            $"Main projects: {FormatInlineList(summaries.Select(summary => summary.RelativePath).Take(6))}",
            $"Entry points: {FormatInlineList(entryFiles)}",
            $"Dashboards/UI: {FormatInlineList(dashboardFiles)}",
            $"Top file roles: {FormatProjectIndexRoleSummary(fileRoles)}",
            $"Top feature areas: {FormatProjectIndexFeatureSummary(featureAreas)}",
    ...
    private IReadOnlyList<string> BuildSymbolStalenessRows(IReadOnlyList<SymbolOwnershipLedgerEntry> ledger)
    {
        if (ledger.Count == 0)
        {
            return ["Good - no symbols were indexed yet"];
        }

        var staleRows = new List<string>();
        foreach (var entry in ledger.Take(120))
        {
            var absolutePath = ToAbsoluteWorkspacePath(entry.RelativePath);
            if (absolutePath is null || !File.Exists(absolutePath))
            {
                staleRows.Add($"Stale - {entry.Symbol}: missing file {entry.RelativePath}");
                continue;
            }

            var lines = SafeReadText(absolutePath).Split([Environment.NewLine, "\n"], StringSplitOptions.None);
            var index = Math.Clamp(entry.LineNumber - 1, 0, Math.Max(0, lines.Length - 1));
            var localWindow = lines
                .Skip(Math.Max(0, index - 2))
                .Take(5)
                .Any(line => line.Contains(entry.Symbol, StringComparison.Ordinal) || line.Contains(entry.Owner, StringComparison.Ordinal));
            if (!localWindow)
            {
                staleRows.Add($"Review - {entry.Symbol}: symbol moved or changed near {entry.RelativePath}:{entry.LineNumber}");
            }
        }

        return staleRows.Count == 0
            ? [$"Good - {ledger.Count} symbol ownership row(s) still match indexed files"]
            : staleRows.Take(12).ToList();
    }

    private static IReadOnlyList<string> BuildProjectIndexRefreshRows(ProjectIndexStatus status)
    {
        if (status.Available)
        {
            return ["Good - project index is current", "Next - refresh only after file count or workspace write changes"];
        }
        ...
        [
            $"Bad - {status.Summary}",
            "Run: project index",
            "Then: rerun the requested coding dashboard/check command"
        ];
    }

    private static IReadOnlyList<string> BuildAutonomousValidationPlanRows(
        string goal,
        ProjectIndexStatus indexStatus,
        TestTargetRecommendation? testTarget)
    {
        var cleanGoal = CleanGoal(goal, "current change");
        var rows = new List<string>
        {
            indexStatus.Available ? "Project index: Good" : "Project index: Bad - run project index first",
            $"Impact scan: show impacted tests {cleanGoal}",
            $"Test resolver: resolve test target {cleanGoal}"
        };

        if (testTarget is not null && !string.IsNullOrWhiteSpace(testTarget.Command))
        {
            rows.Add($"Targeted test: {testTarget.Command}");
        }
        else
        {
            rows.Add("Targeted test: resolve before applying edits");
        }

        rows.Add("Build: confirm dotnet build \"<primary target>\"");
        rows.Add("Closeout: review current changes, then can i safely commit");
        return rows;
    }

    private static EditImpactScore BuildEditImpactScore(
        IReadOnlyList<EditTargetCandidate> rankedTargets,
        ...
        TestTargetRecommendation? testTarget)
    {
        ...
        {
            score += Math.Min(35, 15 + rankedTargets[0].Score * 2);
            reasons.Add($"top target {rankedTargets[0].RelativePath} score {rankedTargets[0].Score}");
        }
        if (hits.Count > 0)
        {
            score += Math.Min(20, hits.Count * 3);
            reasons.Add($"{hits.Count} semantic hit(s)");
        }
        if (candidateFiles.Count > 0)
        {
            score += Math.Min(15, candidateFiles.Count * 2);
            reasons.Add($"{candidateFiles.Count} candidate file(s)");
        }
        if (testTarget is not null && !string.IsNullOrWhiteSpace(testTarget.Command))
        {
            score += 20;
            reasons.Add("targeted test command resolved");
        }
        if (testTarget is not null && testTarget.BuildOrderProjects.Count > 0)
        {
            score += 10;
            reasons.Add("build order slice resolved");
        }

        var confidence = score >= 80 ? "High" : score >= 55 ? "Medium" : score >= 30 ? "Low" : "Needs review";
        return new EditImpactScore(Math.Min(score, 100), confidence, reasons.Count == 0 ? ["no deterministic impact signals"] : reasons.Take(5).ToList());
    }

    private static IReadOnlyList<string> BuildRouteRepairPacketRows(
    ...
    {
        var rows = new List<string>();
        rows.AddRange(missingService.Take(4).Select(action => $"Service repair: add or verify LocalCodingToolService handler for {action}"));
        rows.AddRange(missingPolicy.Take(4).Select(action => $"Policy repair: classify CodingToolAction.{action} in CodingWorkspacePolicy"));
        rows.AddRange(missingParserOrTests.Take(4).Select(action => $"Parser/test repair: add parser phrase or regression assertion for {action}"));
        rows.AddRange(missingDashboardTargets.Take(4).Select(command => $"Dashboard repair: bind {command} to a MainWindowViewModel command"));
        return rows.Count == 0
            ? ["Good - no route repair packets needed"]
            : rows.Take(10).ToList();
    }
    ...
    private async Task<CodingToolResult> ShowProjectIndexAsync(CancellationToken cancellationToken)
    {
        ...
        var symbolStalenessRows = BuildSymbolStalenessRows(symbolOwnershipLedger);
        var focusedTestPlan = BuildFocusedTestRunnerRecommendations(testCommands, summaries);
        var refreshAutomationRows = BuildProjectIndexRefreshRows(new ProjectIndexStatus(true, "Good - index freshly rebuilt."));
        ...
        var lines = new List<string>
        {
            ...
            $"Project intent: {FormatInlineList(projectIntent.Take(5))}",
            $"Compressed repo map: {FormatInlineList(compressedRepoMap.Take(5))}",
            ...
            $"Per-symbol stale detection: {FormatInlineList(symbolStalenessRows.Take(4))}",
            $"Project index refresh automation: {FormatInlineList(refreshAutomationRows.Take(3))}",
    ...
    private async Task<CodingToolResult> DraftReleaseNotesAsync(CancellationToken cancellationToken)
    {
        ...
        {
            foreach (var area in releaseAreas)
            {
                lines.Add($"- Improved {area}.");
            }
        }
        ...
        lines.Add(projectImpact.AffectedProjects.Count == 0
            ...
            : $"- Affected projects: {FormatInlineList(projectImpact.AffectedProjects)}");
        lines.Add(riskLabels.Count == 0
            ? "- Risk labels: none"
            : $"- Risk labels: {FormatInlineList(riskLabels)}");
        lines.Add("Validation:");
    ...
    private async Task<CodingToolResult> ShowMiniCodexStatusAsync(CancellationToken cancellationToken)
    {
        ...
        var scores = new (string Name, int Score, string Note)[]
        {
            ...
            ("Dashboard usability", 97, "compact programming dashboard, current project path picker, new-project location chooser, active workspace button, apply-packet buttons, coding cockpit lane, command queue dashboard rows, status-only command rows, route diff repair packets, and one-click score v3")
        };
        var overall = 99;
        var lines = new List<string>
        {
    ...
    private async Task<FeatureWorkContext> BuildFeatureWorkContextAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var goal = CleanGoal(request.Query, "current feature");
        var changedFiles = await ReadChangedFilesAsync(cancellationToken).ConfigureAwait(false);
        ...
        var riskLabels = relativeCandidates.Select(ClassifyFileRisk).Distinct(StringComparer.OrdinalIgnoreCase).Take(6).ToList();
        var validationRows = BuildValidationQueueRows(goal, changedFiles, testTarget).Take(6).ToList();
        return new FeatureWorkContext(
            goal,
            goalTerms,
            changedFiles,
            graph,
    ...
    private CodingToolResult BuildFeatureIntentPacket(CodingToolRequest request, FeatureWorkContext? context = null)
    {
        var goal = context?.Goal ?? CleanGoal(request.Query, "current feature");
        var files = context is not null
            ? context.RelativeCandidateFiles.Take(8).ToList()
            : SuggestLikelyFilesForGoal(goal).Take(8).ToList();
        var terms = context is not null
            ? context.GoalTerms.Take(6).ToList()
            : ExtractMeaningfulGoalTerms(goal).Take(6).ToList();
    ...
    private async Task<CodingToolResult> PlanBehaviorTestsAsync(
        CodingToolRequest request,
        CancellationToken cancellationToken,
        FeatureWorkContext? context = null)
    {
        context ??= await BuildFeatureWorkContextAsync(request, cancellationToken).ConfigureAwait(false);
        var goal = context.Goal;
        var files = context.ChangedFiles.Count > 0 ? context.ChangedFiles.Take(8).ToList() : context.RelativeCandidateFiles.Take(8).ToList();
        var likelyTests = files
            .Where(IsTestFile)
            .Concat(context.TestTarget.TestFiles.Select(RelativeToWorkspace))
        ...
        lines.AddRange(context.TestTarget.PrioritizedTests.Count == 0 ? ["- none resolved yet"] : context.TestTarget.PrioritizedTests.Take(6).Select(row => $"- {row}"));
        lines.Add("Validation commands:");
        ...
        FeatureWorkContext? context = null)
    {
        ...
        lines.Add("- 1. Behavior contract - define user action, expected result, and failure behavior; confidence high; risk low.");
        lines.AddRange(BuildPatchSliceRows(context).Take(3).Select((row, index) => $"- {index + 2}. {row}"));
        lines.Add("- 5. Apply gate - require target confidence, tests, and protected-file checks before preview/apply; confidence high; risk low.");
        lines.Add("- 6. Validation - run targeted tests, build, review diff, then safe commit check; confidence high; risk low.");
        lines.Add("Candidate files:");
        lines.AddRange(context.RelativeCandidateFiles.Count == 0 ? ["- none detected yet"] : context.RelativeCandidateFiles.Select(file => $"- {file}: {ClassifyFileRisk(file)}"));
        lines.Add($"Risk-aware test depth: {ClassifyRiskAwareTestDepth(context.RelativeCandidateFiles)}");
    ...
    private async Task<CodingToolResult> ShowTestStubGeneratorPlanAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var context = await BuildFeatureWorkContextAsync(request, cancellationToken).ConfigureAwait(false);
        var lines = new List<string>
        {
        ...
        lines.AddRange(BuildTestStubRows(context).Select(row => $"- {row}"));
        lines.AddRange(context.TestTarget.PrioritizedTests.Take(4).Select(row => $"- Targeted test support: {row}"));
        lines.AddRange([
    ...
    private async Task<CodingToolResult> ShowFeatureWorkContextAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var context = await BuildFeatureWorkContextAsync(request, cancellationToken).ConfigureAwait(false);
        var topTarget = context.RankedTargets.FirstOrDefault();
        var lines = new List<string>
        {
            "Feature work context v1:",
            "No files were changed.",
            $"Goal: {context.Goal}",
        ...
        lines.AddRange(context.ValidationRows.Select(row => $"- {row}"));
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Feature work context", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowBehaviorContractAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var context = await BuildFeatureWorkContextAsync(request, cancellationToken).ConfigureAwait(false);
        var lines = new List<string>
        {
        ...
        lines.AddRange(BuildBehaviorContractRows(context).Select(row => $"- {row}"));
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Behavior contract", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowPatchSlicePlanAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var context = await BuildFeatureWorkContextAsync(request, cancellationToken).ConfigureAwait(false);
        var lines = new List<string>
        {
        ...
        lines.AddRange(BuildPatchSliceRows(context).Select(row => $"- {row}"));
        lines.Add("Validation rows:");
        lines.AddRange(context.ValidationRows.Select(row => $"- {row}"));
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Patch slice plan", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowApplyGateAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var context = await BuildFeatureWorkContextAsync(request, cancellationToken).ConfigureAwait(false);
        var lines = new List<string>
        {
        ...
        lines.AddRange(BuildApplyGateRows(context).Select(row => $"- {row}"));
    ...
    private async Task<CodingToolResult> ShowFeatureSessionLedgerAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var goal = CleanGoal(request.Query ?? _lastFeatureBuildGoal, "current feature");
        var context = await BuildFeatureWorkContextAsync(request with { Query = goal }, cancellationToken).ConfigureAwait(false);
        var synthesis = _lastFeaturePatchSynthesis is not null && string.Equals(_lastFeatureBuildGoal, goal, StringComparison.OrdinalIgnoreCase)
        ...
        var gitStatus = await InspectGitWorkingTreeAsync(cancellationToken).ConfigureAwait(false);
        var stage = ClassifyFeatureSessionStage(context, synthesis, latestValidation, latestEdit, gitStatus);
        var score = BuildFeatureSessionScore(context, synthesis, latestValidation, latestEdit, gitStatus);
        var lines = new List<string>
        {
            "Feature session ledger v1:",
            "No files were changed.",
            $"Goal: {goal}",
            $"Current stage: {stage}",
        ...
        lines.AddRange(BuildFeatureSessionChecklistRows(context, synthesis, latestValidation, latestEdit, gitStatus).Select(row => $"- {row}"));
        lines.Add("Evidence:");
        lines.AddRange(BuildFeatureSessionEvidenceRows(context, latestValidation, latestEdit, latestGit, gitStatus).Select(row => $"- {row}"));
        lines.Add($"Next command: {BuildFeatureSessionNextCommand(goal, context, synthesis, latestValidation, latestEdit)}");
        lines.Add("Owner-safe rule: preview first, apply only after the visible diff matches the request, then validate before closeout.");
    ...
    private async Task<CodingToolResult> ShowFeatureRunControllerAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        ...
        var latestGit = receipts.LastOrDefault(IsFeatureGitReceipt);
        var gitStatus = await InspectGitWorkingTreeAsync(cancellationToken).ConfigureAwait(false);
        var phase = ClassifyFeatureRunControllerPhase(context, synthesis, latestValidation, latestEdit, gitStatus);
        var nextCommand = BuildFeatureRunControllerNextCommand(goal, context, synthesis, latestValidation, latestEdit, gitStatus);
        var lines = new List<string>
        {
            "Feature run controller v1:",
            "No files were changed.",
            $"Goal: {goal}",
            $"Phase: {phase}",
            $"Go/no-go: {BuildFeatureRunGoNoGo(context, synthesis, latestValidation, latestEdit, gitStatus)}",
            $"Controller score: {BuildFeatureRunControllerScore(context, synthesis, latestValidation, latestEdit, gitStatus)}/100",
            latestValidation is null ? "Latest validation: none" : FormatReceiptSummary("Latest validation", latestValidation),
            latestEdit is null ? "Latest edit: none" : FormatReceiptSummary("Latest edit", latestEdit),
            latestGit is null ? "Latest git check: none" : FormatReceiptSummary("Latest git check", latestGit),
            $"Git: {gitStatus.Summary}",
        ...
        lines.AddRange(BuildFeatureRunStateRows(context, synthesis, latestValidation, latestEdit).Select(row => $"- {row}"));
        lines.Add("Stop gates:");
        lines.AddRange(BuildFeatureRunStopGateRows(context, latestValidation, gitStatus).Select(row => $"- {row}"));
        lines.Add("Owner controls:");
        lines.Add("- Preview: show pending patch preview or preview synthesized feature patch.");
        lines.Add("- Apply: confirm apply last patch preview only after the visible diff is right.");
        lines.Add("- Validate: run the validation router or targeted build/test command.");
        lines.Add("- Repair: use validation repair runner if validation fails.");
        lines.Add("- Closeout: feature completion receipt, review current changes, then safe commit check.");
        lines.Add($"Next command: {nextCommand}");
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Feature run controller", Policy.WorkspaceRoot);
    }
    ...
    private async Task<CodingToolResult> ShowBuildThisFeatureAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var goal = CleanGoal(request.Query ?? _lastFeatureBuildGoal, "current feature");
        var context = await BuildFeatureWorkContextAsync(request with { Query = goal }, cancellationToken).ConfigureAwait(false);
        var synthesis = await BuildFeaturePatchSynthesisAsync(context, cancellationToken).ConfigureAwait(false);
        ...
        var lines = new List<string>
        {
            "Build this feature v1:",
            "No files were changed.",
            $"Request: {goal}",
            $"Stage: {ClassifyGuidedFeatureWorkflowStage(context, synthesis, testPreview, latestValidation, latestEdit, gitStatus)}",
            $"Readiness score: {BuildAutonomousFeatureOrchestratorScore(context, synthesis, testPreview, latestValidation, latestEdit, gitStatus, _lastPatchPreviewRequest is not null)}/100",
            "Front-door plan:"
        };
        lines.AddRange(BuildBuildThisFeatureRows(context, synthesis, testPreview).Select(row => $"- {row}"));
        lines.Add("Execution route:");
        lines.AddRange(BuildAutonomousFeatureCycleRows(context, synthesis, testPreview, latestValidation, latestEdit, gitStatus, _lastPatchPreviewRequest is not null).Take(8).Select(row => $"- {row}"));
        lines.Add("Repair route:");
        lines.AddRange(BuildPostApplyRepairRows(goal, latestValidation, _lastPatchPreviewRequest is not null).Select(row => $"- {row}"));
        lines.Add($"Next command: {nextCommand}");
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Build this feature", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowRoslynEditPlannerV2Async(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var context = await BuildFeatureWorkContextAsync(request, cancellationToken).ConfigureAwait(false);
        var lines = new List<string>
        {
        ...
        lines.AddRange(BuildRoslynEditTargetRows(context).Select(row => $"- {row}"));
        lines.Add("Edit order:");
        lines.AddRange(BuildRoslynEditOrderRows(context).Select(row => $"- {row}"));
        lines.Add("Call and reference guard:");
        lines.AddRange(BuildFeatureRoslynTargetingRows(context).Select(row => $"- {row}"));
        lines.Add($"Next command: multi-file patch synthesis {context.Goal}");
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Roslyn edit planner v2", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowMultiFilePatchSynthesisV2Async(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var context = await BuildFeatureWorkContextAsync(request, cancellationToken).ConfigureAwait(false);
        var synthesis = await BuildFeaturePatchSynthesisAsync(context, cancellationToken).ConfigureAwait(false);
        var testPreview = await BuildBehaviorTestPatchReadinessAsync(context, cancellationToken).ConfigureAwait(false);
        ...
        var lines = new List<string>
        {
            ...
            "No files were changed.",
            $"Goal: {context.Goal}",
            $"Preview status: {synthesis.Status}",
            "Bundle candidates:"
        };
        lines.AddRange(BuildMultiFilePatchBundleRows(context, synthesis, testPreview).Select(row => $"- {row}"));
        lines.Add("Cross-file roles:");
        lines.AddRange(BuildFeatureImplementationFileRoleRows(context).Take(8).Select(row => $"- {row}"));
        lines.Add("Preview route:");
        lines.AddRange(BuildMultiFilePatchPreviewRouteRows(context, synthesis, testPreview).Select(row => $"- {row}"));
        lines.Add($"Next command: {BuildGuidedFeatureImplementationNextCommand(context, synthesis, testPreview)}");
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Multi-file patch synthesis v2", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowPatternCopyPlanAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var context = await BuildFeatureWorkContextAsync(request, cancellationToken).ConfigureAwait(false);
        await Task.CompletedTask.ConfigureAwait(false);
        var lines = new List<string>
        {
            "Pattern copy plan v1:",
            "No files were changed.",
            $"Goal: {context.Goal}",
            "Nearby patterns:"
        };
        lines.AddRange(BuildPatternCopyRows(context).Select(row => $"- {row}"));
        lines.Add("Copy rules:");
        lines.AddRange(BuildPatternCopyRuleRows(context).Select(row => $"- {row}"));
        lines.Add($"Next command: roslyn edit planner {context.Goal}");
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Pattern copy plan", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowBehaviorTestGeneratorV2Async(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var context = await BuildFeatureWorkContextAsync(request, cancellationToken).ConfigureAwait(false);
        var edit = await BuildBehaviorTestPatchPreviewEditAsync(context, cancellationToken).ConfigureAwait(false);
        var lines = new List<string>
        {
            ...
            "No files were changed.",
            $"Goal: {context.Goal}",
            edit.Ready ? "Status: Good - behavior test preview can be generated." : $"Status: Review - {edit.Status}",
            "Generated shape:"
        };
        lines.AddRange(BuildBehaviorTestGeneratorRows(context, edit).Select(row => $"- {row}"));
        lines.Add("Assertion guidance:");
        lines.AddRange(BuildBehaviorAssertionGuidanceRows(context).Select(row => $"- {row}"));
        lines.Add($"Next command: {(edit.Ready ? edit.Command : "behavior test plan " + context.Goal)}");
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Behavior test generator v2", edit.FullPath ?? Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowImplementationSliceStateAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var goal = CleanGoal(request.Query ?? _lastFeatureBuildGoal, "current feature");
        var context = await BuildFeatureWorkContextAsync(request with { Query = goal }, cancellationToken).ConfigureAwait(false);
        var synthesis = _lastFeaturePatchSynthesis is not null && string.Equals(_lastFeatureBuildGoal, goal, StringComparison.OrdinalIgnoreCase)
        ...
        var lines = new List<string>
        {
            "Implementation slice state v1:",
            "No files were changed.",
            $"Goal: {goal}",
            "Slices:"
        };
        lines.AddRange(BuildImplementationSliceStateRows(context, synthesis, testPreview, latestValidation, latestEdit, gitStatus, _lastPatchPreviewRequest is not null).Select(row => $"- {row}"));
        lines.Add("State memory:");
        lines.AddRange(BuildImplementationSliceMemoryRows(context, receipts).Select(row => $"- {row}"));
        lines.Add($"Next command: {BuildGuidedFeatureWorkflowNextCommand(goal, context, synthesis, testPreview, latestValidation, latestEdit, gitStatus)}");
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Implementation slice state", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowPostApplyRepairLoopV2Async(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var goal = CleanGoal(request.Query ?? _lastFeatureBuildGoal, "current feature");
        var context = await BuildFeatureWorkContextAsync(request with { Query = goal }, cancellationToken).ConfigureAwait(false);
        LoadPendingPatchPreviewIfNeeded();
        ...
        lines.AddRange(BuildPostApplyRepairRows(goal, latestValidation, _lastPatchPreviewRequest is not null).Select(row => $"- {row}"));
        lines.Add("Likely scope:");
        lines.AddRange(BuildValidationRepairFocusRows(context, latestValidation?.Action ?? "no failed validation", new CompilerDiagnosticInfo(null, null, null), null, null).Take(4).Select(row => $"- {row}"));
        lines.Add($"Next command: {BuildPostApplyRepairNextCommand(goal, latestValidation, _lastPatchPreviewRequest is not null)}");
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Post-apply repair loop", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowSemanticDiffSummaryAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var goal = CleanGoal(request.Query ?? _lastFeatureBuildGoal, "current feature");
        var changedFiles = await ReadChangedFilesAsync(cancellationToken).ConfigureAwait(false);
        LoadPendingPatchPreviewIfNeeded();
        var projectImpact = BuildProjectImpact(changedFiles.Select(ToAbsoluteWorkspacePath).Where(path => !string.IsNullOrWhiteSpace(path)).Select(path => path!).ToList(), GetWorkspaceProjectSummaries());
        var lines = new List<string>
        {
            "Semantic diff summary v1:",
            "No files were changed.",
            $"Goal: {goal}",
            $"Changed files: {changedFiles.Count}",
            _lastPatchPreviewRequest is null ? "Pending preview: none" : $"Pending preview: {_lastPatchPreviewRequest.Action}",
        ...
        lines.AddRange(BuildSemanticDiffSummaryRows(changedFiles, projectImpact).Select(row => $"- {row}"));
        lines.Add("Patch checks:");
        lines.AddRange(changedFiles.Count == 0 ? ["- Waiting - no changed files to inspect."] : BuildSemanticPatchCheckLines(changedFiles).Take(6));
        lines.Add($"Next command: {(changedFiles.Count == 0 ? "autonomous feature orchestrator " + goal : "post patch validation " + goal)}");
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Semantic diff summary", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowMiniCodexScoreV3Async(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var goal = CleanGoal(request.Query ?? _lastFeatureBuildGoal, "current feature");
        var context = await BuildFeatureWorkContextAsync(request with { Query = goal }, cancellationToken).ConfigureAwait(false);
        var synthesis = _lastFeaturePatchSynthesis is not null && string.Equals(_lastFeatureBuildGoal, goal, StringComparison.OrdinalIgnoreCase)
        ...
        var latestEdit = receipts.LastOrDefault(IsFeatureEditReceipt);
        var gitStatus = await InspectGitWorkingTreeAsync(cancellationToken).ConfigureAwait(false);
        var scores = BuildMiniCodexScoreV3Rows(context, synthesis, testPreview, latestValidation, latestEdit, gitStatus, _lastPatchPreviewRequest is not null).ToList();
        var overall = (int)Math.Round(scores.Select(row => row.Score).DefaultIfEmpty(0).Average(), MidpointRounding.AwayFromZero);
        var lines = new List<string>
        {
            "Mini-Codex score v3:",
            "No files were changed.",
        ...
        lines.AddRange(scores.Select(row => $"- {row.Name}: {row.Score}% - {row.Note}"));
        lines.Add("Next score move:");
        lines.AddRange(BuildMiniCodexScoreNextRows(scores, context.Goal).Select(row => $"- {row}"));
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Mini-Codex score v3", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowConcretePatchAuthoringAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var goal = CleanGoal(request.Query ?? _lastFeatureBuildGoal, "current feature");
        var context = await BuildFeatureWorkContextAsync(request with { Query = goal }, cancellationToken).ConfigureAwait(false);
        var synthesis = await BuildFeaturePatchSynthesisAsync(context, cancellationToken).ConfigureAwait(false);
        ...
        var lines = new List<string>
        {
            "Concrete patch authoring v1:",
            "No files were changed.",
            $"Goal: {goal}",
            $"Authoring confidence: {BuildPatchConfidenceScore(context, synthesis, testPreview, latestValidation, latestEdit, gitStatus)}/100",
            "Authoring pipeline:"
        };
        lines.AddRange(BuildConcreteAuthoringPipelineRows(context, synthesis, testPreview, latestValidation, gitStatus).Select(row => $"- {row}"));
        lines.Add("Patch body readiness:");
        lines.AddRange(BuildPatchBodyCandidateRows(synthesis).Select(row => $"- {row}"));
        lines.Add("Review gates:");
        lines.AddRange(BuildPatchConfidenceRiskRows(context, synthesis, testPreview, latestValidation, gitStatus).Select(row => $"- {row}"));
        lines.Add($"Next command: patch body generator {goal}");
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Concrete patch authoring", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowPatchBodyGeneratorAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var goal = CleanGoal(request.Query ?? _lastFeatureBuildGoal, "current feature");
        var context = await BuildFeatureWorkContextAsync(request with { Query = goal }, cancellationToken).ConfigureAwait(false);
        var synthesis = _lastFeaturePatchSynthesis is not null && string.Equals(_lastFeatureBuildGoal, goal, StringComparison.OrdinalIgnoreCase)
        ...
        var lines = new List<string>
        {
            "Patch body generator v1:",
            "No files were changed.",
            $"Goal: {goal}",
            $"Body status: {synthesis.Status}",
            "Candidate patch bodies:"
        };
        lines.AddRange(BuildPatchBodyCandidateRows(synthesis).Select(row => $"- {row}"));
        lines.Add("Exact anchors:");
        lines.AddRange(BuildPatchAnchorRows(synthesis).Select(row => $"- {row}"));
        lines.Add("Patch guards:");
        lines.AddRange(synthesis.GuardRows.Take(5).Select(row => $"- {row}"));
        lines.Add($"Next command: patch confidence score {goal}");
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Patch body generator", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowPatternCommandScaffolderAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var context = await BuildFeatureWorkContextAsync(request, cancellationToken).ConfigureAwait(false);
        var lines = new List<string>
        {
            "Pattern command scaffolder v1:",
            "No files were changed.",
            $"Goal: {context.Goal}",
            "Scaffold roles:"
        };
        lines.AddRange(BuildCommandScaffoldRoleRows(context).Select(row => $"- {row}"));
        lines.Add("Pattern sources:");
        lines.AddRange(BuildPatternCopyRows(context).Take(6).Select(row => $"- {row}"));
        lines.Add("Route checklist:");
        lines.AddRange(BuildPatternCopyRuleRows(context).Select(row => $"- {row}"));
        lines.Add($"Next command: ui bundle planner {context.Goal}");
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Pattern command scaffolder", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowUiBundlePlannerAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var context = await BuildFeatureWorkContextAsync(request, cancellationToken).ConfigureAwait(false);
        var synthesis = await BuildFeaturePatchSynthesisAsync(context, cancellationToken).ConfigureAwait(false);
        var testPreview = await BuildBehaviorTestPatchReadinessAsync(context, cancellationToken).ConfigureAwait(false);
        ...
        var lines = new List<string>
        {
            "UI bundle planner v1:",
            "No files were changed.",
            $"Goal: {context.Goal}",
            "Bundle roles:"
        };
        lines.AddRange(BuildUiBundleRoleRows(context).Select(row => $"- {row}"));
        lines.Add("UI binding checks:");
        lines.AddRange(BuildMultiFilePatchBundleRows(context, synthesis, testPreview).Take(6).Select(row => $"- {row}"));
        lines.Add("Validation route:");
        lines.AddRange(BuildValidationCommandRows(context, null).Select(row => $"- {row}"));
        lines.Add($"Next command: multi-file patch synthesis {context.Goal}");
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "UI bundle planner", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowPatchConfidenceScoreAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var goal = CleanGoal(request.Query ?? _lastFeatureBuildGoal, "current feature");
        var context = await BuildFeatureWorkContextAsync(request with { Query = goal }, cancellationToken).ConfigureAwait(false);
        var synthesis = _lastFeaturePatchSynthesis is not null && string.Equals(_lastFeatureBuildGoal, goal, StringComparison.OrdinalIgnoreCase)
        ...
        var lines = new List<string>
        {
            "Patch confidence score v1:",
            "No files were changed.",
            $"Goal: {goal}",
            $"Confidence: {confidence}/100",
            "Confidence rows:"
        };
        lines.AddRange(BuildPatchConfidenceRows(context, synthesis, testPreview, latestValidation, latestEdit, gitStatus).Select(row => $"- {row}"));
        lines.Add("Risk gates:");
        lines.AddRange(BuildPatchConfidenceRiskRows(context, synthesis, testPreview, latestValidation, gitStatus).Select(row => $"- {row}"));
        lines.Add($"Next command: slice executor preview {goal}");
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Patch confidence score", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowSliceExecutorPreviewAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var goal = CleanGoal(request.Query ?? _lastFeatureBuildGoal, "current feature");
        var context = await BuildFeatureWorkContextAsync(request with { Query = goal }, cancellationToken).ConfigureAwait(false);
        var synthesis = _lastFeaturePatchSynthesis is not null && string.Equals(_lastFeatureBuildGoal, goal, StringComparison.OrdinalIgnoreCase)
        ...
        var lines = new List<string>
        {
            "Slice executor preview v1:",
            "No files were changed.",
            $"Goal: {goal}",
            "Preview slices:"
        };
        lines.AddRange(BuildSliceExecutorPreviewRows(context, synthesis, testPreview).Select(row => $"- {row}"));
        lines.Add("Apply boundaries:");
        lines.AddRange(BuildSliceApplyBoundaryRows(context, synthesis).Select(row => $"- {row}"));
        lines.Add($"Next command: {(synthesis.PreviewReady && testPreview.Ready ? "preview guided feature bundle " : "patch body generator ")}{goal}");
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Slice executor preview", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowFailureToPatchV3Async(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var goal = CleanGoal(request.Query ?? _lastFeatureBuildGoal, "current feature");
        var context = await BuildFeatureWorkContextAsync(request with { Query = goal }, cancellationToken).ConfigureAwait(false);
        LoadPendingPatchPreviewIfNeeded();
        ...
        lines.AddRange(BuildFailureToPatchStateRows(latestValidation, gitStatus, _lastPatchPreviewRequest is not null).Select(row => $"- {row}"));
        lines.Add("Patch route:");
        lines.AddRange(BuildFailureToPatchRouteRows(context, latestValidation).Select(row => $"- {row}"));
        lines.Add("Repair guards:");
        lines.AddRange(BuildPostApplyRepairRows(goal, latestValidation, _lastPatchPreviewRequest is not null).Select(row => $"- {row}"));
        lines.Add($"Next command: {BuildPostApplyRepairNextCommand(goal, latestValidation, _lastPatchPreviewRequest is not null)}");
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Failure-to-patch v3", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowSemanticChangeReceiptAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var goal = CleanGoal(request.Query ?? _lastFeatureBuildGoal, "current feature");
        var changedFiles = await ReadChangedFilesAsync(cancellationToken).ConfigureAwait(false);
        var receipts = ReadRecentReceipts(MaxReceiptEntries);
        ...
        var lines = new List<string>
        {
            "Semantic change receipt v1:",
            "No files were changed.",
            $"Goal: {goal}",
            "Receipt rows:"
        };
        lines.AddRange(BuildSemanticChangeReceiptRows(changedFiles, latestValidation, latestEdit, latestGit).Select(row => $"- {row}"));
        lines.Add("Semantic changes:");
        lines.AddRange(BuildSemanticDiffSummaryRows(changedFiles, projectImpact).Select(row => $"- {row}"));
        lines.Add("Closeout:");
        lines.AddRange(BuildValidationCommandRows(null, latestValidation).Select(row => $"- {row}"));
        lines.Add($"Next command: validation chain planner {goal}");
        return new CodingToolResult(true, true, string.Join(Environment.NewLine, lines), "Semantic change receipt", Policy.WorkspaceRoot);
    }

    private async Task<CodingToolResult> ShowValidationChainPlannerAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var goal = CleanGoal(request.Query ?? _lastFeatureBuildGoal, "current feature");
        var context = await BuildFeatureWorkContextAsync(request with { Query = goal }, cancellationToken).ConfigureAwait(false);
        var receipts = ReadRecentReceipts(MaxReceiptEntries);
        ...
        var lines = new List<string>
        {
            "Validation chain planner v1:",
            "No files were changed.",
            $"Goal: {goal}",
            "Validation order:"
        };
        lines.AddRange(BuildValidationChainRows(context, latestValidation, gitStatus).Select(row => $"- {row}"));
        lines.Add("Commands:");
        lines.AddRange(BuildValidationCommandRows(context, latestValidation).Select(row => $"- {row}"));
        lines.Add("Repair fallback:");
        lines.AddRange(BuildPostApplyRepairRows(goal, latestValidation, _lastPatchPreviewRequest is not null).Select(row => $"- {row}"));
        lines.Add($"Next command: {(latestValidation?.Succeeded == true ? "semantic change receipt " : "post patch validation ")}{goal}");
    ...
    private static IReadOnlyList<string> BuildBuildThisFeatureRows(
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        BehaviorTestPatchReadiness testPreview)
    {
        return
        [
            $"Intake: Good - normalized request is {context.Goal}.",
            context.RankedTargets.Count == 0 ? "Target: Review - no primary target yet." : $"Target: Good - {context.RankedTargets[0].RelativePath}.",
            $"Roslyn plan: roslyn edit planner {context.Goal}.",
            $"Pattern copy: pattern copy {context.Goal}.",
            testPreview.Ready ? $"Test generator: Good - {testPreview.Command}." : $"Test generator: Review - {testPreview.Status}",
            synthesis.PreviewReady ? $"Patch synthesis: Good - {synthesis.PreviewEdits.Count} preview edit(s)." : $"Patch synthesis: Review - {synthesis.Status}",
            "Validation: run only through confirmation after apply."
        ];
    }

    private IReadOnlyList<string> BuildRoslynEditTargetRows(FeatureWorkContext context)
    {
        var declarations = context.Graph.Declarations
            .Take(5)
            .Select(hit => $"{hit.Kind} {hit.Display} at {RelativeToWorkspace(hit.Path)}:{hit.LineNumber}")
            .ToList();
        var references = context.Graph.References
            .Take(5)
            .Select(hit => $"{hit.Kind} {hit.Display} at {RelativeToWorkspace(hit.Path)}:{hit.LineNumber}")
            .ToList();
        return
        [
            declarations.Count == 0 ? "Declarations: Review - none matched the request terms." : $"Declarations: Good - {FormatInlineList(declarations)}",
            references.Count == 0 ? "References: Review - none matched the request terms." : $"References: Good - {FormatInlineList(references)}",
            $"Ranked files: {FormatInlineList(context.RankedTargets.Take(5).Select(target => $"{target.RelativePath} ({target.Confidence})"))}",
            $"Reference files: {FormatInlineList(context.Graph.ReferenceFiles.Take(8))}"
        ];
    }

    private static IReadOnlyList<string> BuildRoslynEditOrderRows(FeatureWorkContext context)
    {
        var ordered = context.RankedTargets
            .OrderBy(target => IsTestFile(target.RelativePath) ? 2 : 1)
            .ThenByDescending(target => target.Score)
            .Take(6)
            .Select((target, index) => $"{index + 1}. {target.RelativePath}: {ClassifyFeatureImplementationRole(target.RelativePath)}; risk {target.Risk}; reason {FormatInlineList(target.Reasons.Take(2))}")
            .ToList();
        return ordered.Count == 0
            ? ["1. Review - inspect workspace/symbol map before editing."]
            : ordered;
    }

    private static IReadOnlyList<string> BuildMultiFilePatchBundleRows(
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        BehaviorTestPatchReadiness testPreview)
    {
        var sourceFiles = context.RankedTargets
        ...
        return
        [
            $"Source edits: {(sourceFiles.Count == 0 ? "Review - no source target." : "Good - " + FormatInlineList(sourceFiles))}",
            testPreview.Ready ? $"Behavior test edit: Good - {testPreview.TargetFile}." : $"Behavior test edit: Review - {testPreview.Status}",
            synthesis.PreviewReady ? $"Exact preview edits: Good - {synthesis.PreviewEdits.Count}." : $"Exact preview edits: Review - {synthesis.Status}",
            $"UI/ViewModel candidates: {FormatInlineList(context.RelativeCandidateFiles.Where(file => file.EndsWith(".xaml", StringComparison.OrdinalIgnoreCase) || file.Contains("ViewModel", StringComparison.OrdinalIgnoreCase)).Take(6))}",
            $"Bundle size guard: max {MaxPatchBundleEdits} exact edits per preview."
        ];
    }

    private static IReadOnlyList<string> BuildMultiFilePatchPreviewRouteRows(
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        BehaviorTestPatchReadiness testPreview)
    {
        ...
        [
            testPreview.Ready && synthesis.PreviewReady
                ? $"Paired preview: preview guided feature bundle {context.Goal}"
                : "Paired preview: Review - needs both behavior-test and code preview readiness.",
            synthesis.PreviewReady
                ? $"Code-only preview: preview synthesized feature patch {context.Goal}"
                : "Code-only preview: Waiting - exact patch text not ready.",
            testPreview.Ready
                ? $"Test-only preview: {testPreview.Command}"
                : "Test-only preview: Waiting - no ready behavior-test edit."
        ];
    }

    private IReadOnlyList<string> BuildPatternCopyRows(FeatureWorkContext context)
    {
        var terms = context.GoalTerms.Take(6).ToList();
        var candidateDirectories = context.CandidateFiles
            .Where(File.Exists)
        ...
        {
            var examples = Directory.EnumerateFiles(directory!, "*.cs", SearchOption.TopDirectoryOnly)
                .Where(file => context.CandidateFiles.All(candidate => !candidate.Equals(file, StringComparison.OrdinalIgnoreCase)))
                .Where(file => terms.Count == 0 || terms.Any(term => Path.GetFileName(file).Contains(term, StringComparison.OrdinalIgnoreCase) || SafeReadText(file).Contains(term, StringComparison.OrdinalIgnoreCase)))
                .Take(2)
                .Select(file => $"{RelativeToWorkspace(file)} ({ClassifyFeatureImplementationRole(file)})")
                .ToList();
            rows.AddRange(examples.Select(example => $"Pattern source: {example}."));
        }

        if (rows.Count == 0)
        {
            rows.AddRange(context.RankedTargets.Take(4).Select(target => $"Pattern source: {target.RelativePath} ({ClassifyFeatureImplementationRole(target.RelativePath)})."));
        }

        return rows.Count == 0 ? ["Pattern source: Review - no nearby pattern found yet."] : rows.Take(8).ToList();
    }

    private static IReadOnlyList<string> BuildPatternCopyRuleRows(FeatureWorkContext context)
    {
        return
        [
            "Copy naming, command binding, dependency injection, and test layout from the closest local example.",
            "Do not copy generated/designer files unless the owner explicitly asked.",
            $"Keep acceptance behavior tied to {context.Goal}.",
            "Preview copied pattern changes as a patch bundle before apply."
        ];
    }

    private static IReadOnlyList<string> BuildBehaviorTestGeneratorRows(
        FeatureWorkContext context,
        BehaviorTestPatchPreviewEdit edit)
    {
        return
        [
            edit.RelativePath is null ? "Target file: Review - none resolved." : $"Target file: {edit.RelativePath}.",
            edit.FrameworkName is null ? "Framework: Review - not detected." : $"Framework: {edit.FrameworkName}.",
            edit.MethodName is null ? "Method: Review - not generated." : $"Method: {edit.MethodName}.",
            edit.AnchorReason is null ? "Anchor: Review - insertion point not resolved." : $"Anchor: {edit.AnchorReason}.",
            $"Contract: {context.Goal} should be proven before closeout."
        ];
    }

    private static IReadOnlyList<string> BuildBehaviorAssertionGuidanceRows(FeatureWorkContext context)
    {
        return
        [
            $"Arrange the existing path that triggers {context.Goal}.",
            "Act through the public command/service surface, not private implementation details.",
            "Assert the visible output or state change the owner asked for.",
            "Leave NotImplementedException only in preview; replace it before relying on the test."
        ];
    }

    private static IReadOnlyList<string> BuildImplementationSliceStateRows(
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        BehaviorTestPatchReadiness testPreview,
        CodingReceipt? latestValidation,
        ...
        bool hasPendingPreview)
    {
        return
        [
            context.RankedTargets.Count == 0 ? "1. Intake/target: Review - no primary target." : $"1. Intake/target: Good - {context.RankedTargets[0].RelativePath}.",
            testPreview.Ready ? $"2. Test slice: Ready - {testPreview.Command}." : $"2. Test slice: Review - {testPreview.Status}",
            synthesis.PreviewReady ? $"3. Code slice: Ready - {synthesis.PreviewEdits.Count} preview edit(s)." : $"3. Code slice: Review - {synthesis.Status}",
            hasPendingPreview ? "4. Apply slice: Waiting - owner decision on pending preview." : "4. Apply slice: Ready - no pending preview is blocking planning.",
            latestEdit is null ? "5. Edit receipt: Waiting - no applied edit." : "5. Edit receipt: Good - applied edit receipt exists.",
            latestValidation?.Succeeded == true ? "6. Validation: Good - latest validation passed." : "6. Validation: Waiting - targeted validation still needed.",
            gitStatus.HasUncommittedChanges ? "7. Closeout: Review - uncommitted changes remain." : "7. Closeout: Good - no uncommitted change detected."
        ];
    }

    private static IReadOnlyList<string> BuildImplementationSliceMemoryRows(
        FeatureWorkContext context,
        IReadOnlyList<CodingReceipt> receipts)
    {
        var recent = receipts.TakeLast(4).Select(receipt => $"{receipt.Action}:{(receipt.Succeeded ? "Good" : "Bad")}").ToList();
        return
        [
            $"Goal memory: {context.Goal}.",
            $"Candidate files: {FormatInlineList(context.RelativeCandidateFiles.Take(6))}",
            $"Recent receipts: {FormatInlineList(recent)}"
        ];
    }

    private static IReadOnlyList<string> BuildPostApplyRepairRows(
        string goal,
        CodingReceipt? latestValidation,
        bool hasPendingPreview)
    {
        if (hasPendingPreview)
        {
            return ["Pending preview: Review - apply or discard before repair routing."];
        }

        if (latestValidation is null)
        {
            return [$"Validation: Waiting - run post patch validation {goal} after apply."];
        }

        if (latestValidation.Succeeded)
        {
            return ["Validation: Good - no repair needed from the latest receipt."];
        }

        return
        [
            "Validation: Bad - route the first useful diagnostic.",
            $"Repair command: validation repair runner {goal}",
            "Patch rule: preview the smallest deterministic repair before apply."
        ];
    }

    private static string BuildPostApplyRepairNextCommand(
        string goal,
        CodingReceipt? latestValidation,
        bool hasPendingPreview)
    {
        ...
        return latestValidation?.Succeeded == true ? "implementation evidence pack " + goal : "post patch validation " + goal;
    }

    private static IReadOnlyList<string> BuildSemanticDiffSummaryRows(
        IReadOnlyList<string> changedFiles,
        ProjectImpact projectImpact)
    {
        if (changedFiles.Count == 0)
        ...
        var grouped = changedFiles
            .GroupBy(ClassifyFeatureImplementationRole, StringComparer.OrdinalIgnoreCase)
            .Select(group => $"{group.Key}: {group.Count()} file(s), {FormatInlineList(group.Take(3))}")
            .Take(6)
            .ToList();
        grouped.Add($"Affected projects: {FormatInlineList(projectImpact.AffectedProjects)}");
        grouped.Add($"Build order: {FormatInlineList(projectImpact.BuildOrderProjects)}");
        grouped.Add($"Risk depth: {ClassifyRiskAwareTestDepth(changedFiles)}");
        return grouped;
    }

    private IReadOnlyList<(string Name, int Score, string Note)> BuildMiniCodexScoreV3Rows(
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        BehaviorTestPatchReadiness testPreview,
        CodingReceipt? latestValidation,
    ...
    private static IReadOnlyList<string> BuildMiniCodexScoreNextRows(
        IReadOnlyList<(string Name, int Score, string Note)> scores,
        string goal)
    {
        var lowest = scores.OrderBy(score => score.Score).FirstOrDefault();
        if (lowest.Name is null)
        {
            return ["Run mini codex readiness report."];
        }
    ...
    private static IReadOnlyList<string> BuildConcreteAuthoringPipelineRows(
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        BehaviorTestPatchReadiness testPreview,
        CodingReceipt? latestValidation,
        GitWorkingTreeStatus gitStatus)
    {
        ...
        return
        [
            topTarget is null ? "Target: Review - no ranked file yet." : $"Target: Good - {topTarget.RelativePath}.",
            context.GoalTerms.Count == 0 ? "Intent terms: Review - goal is too broad." : $"Intent terms: Good - {FormatInlineList(context.GoalTerms.Take(6))}.",
            synthesis.Candidates.Count == 0 ? "Patch anchor: Waiting - no candidate anchor found." : $"Patch anchor: Good - {synthesis.Candidates.Count} candidate(s).",
            ...
            testPreview.Ready ? $"Behavior test: Good - {testPreview.Command}." : $"Behavior test: Review - {testPreview.Status}",
            latestValidation?.Succeeded == true ? "Validation receipt: Good - latest validation passed." : "Validation receipt: Waiting - run validation after preview/apply.",
            gitStatus.Available ? $"Git state: {gitStatus.Summary}." : "Git state: Review - status unavailable."
        ];
    }

    private static IReadOnlyList<string> BuildPatchBodyCandidateRows(FeaturePatchSynthesis synthesis)
    {
        if (synthesis.Candidates.Count == 0)
        {
            return ["Patch body: Waiting - no candidate file or exact anchor resolved yet."];
        }

        return synthesis.Candidates
            .Take(6)
            .Select(candidate => candidate.PreviewReady
                ? $"Patch body: Ready - {candidate.RelativePath}; exact old/new block available."
                : $"Patch body: Draft - {candidate.RelativePath}; {candidate.Note}")
            .ToList();
    }

    private static IReadOnlyList<string> BuildPatchAnchorRows(FeaturePatchSynthesis synthesis)
    {
        if (synthesis.Candidates.Count == 0)
        {
            return ["Anchor: Waiting - inspect target file before writing patch text."];
        }

        return synthesis.Candidates
            .Take(6)
            .Select(candidate => $"Anchor: {candidate.RelativePath} - {candidate.AnchorReason}.")
            .ToList();
    }

    private static IReadOnlyList<string> BuildCommandScaffoldRoleRows(FeatureWorkContext context)
    {
        var topTarget = context.RankedTargets.FirstOrDefault()?.RelativePath ?? "top resolved source file";
        return
        [
            $"Parser route: add owner phrase for {context.Goal}.",
            "Policy row: mark planning/scaffold command read-only.",
            $"Service handler: return compact rows from {topTarget}.",
            "Dashboard binding: add a button only if the command belongs in the cockpit.",
            "Test route: parser, service heading, policy read-only, dashboard binding."
        ];
    }

    private static IReadOnlyList<string> BuildUiBundleRoleRows(FeatureWorkContext context)
    {
        ...
        return
        [
            uiFiles.Count == 0 ? "View: Review - no XAML/window target found." : $"View: Good - {FormatInlineList(uiFiles)}.",
            context.RelativeCandidateFiles.Any(file => file.Contains("ViewModel", StringComparison.OrdinalIgnoreCase))
                ? "ViewModel: Good - candidate binding surface found."
                : "ViewModel: Review - verify command/property binding manually.",
            "Command: verify CanExecute refresh, compact output, and owner label.",
            "Service: keep the dashboard button tied to a deterministic coding command.",
            "Tests: scan XAML label, command binding, and view-model command string."
        ];
    }

    private static IReadOnlyList<string> BuildPatchConfidenceRows(
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        BehaviorTestPatchReadiness testPreview,
        CodingReceipt? latestValidation,
        ...
        GitWorkingTreeStatus gitStatus)
    {
        return
        [
            context.RankedTargets.Count > 0 ? $"Target confidence: Good - {context.ImpactScore.Score}/100." : "Target confidence: Review - no ranked target.",
            synthesis.PreviewReady ? "Patch body confidence: Good - exact preview exists." : $"Patch body confidence: Review - {synthesis.Status}",
            testPreview.Ready ? "Test confidence: Good - behavior test preview route exists." : $"Test confidence: Review - {testPreview.Status}",
            latestEdit is null ? "Edit receipt: Waiting - no applied edit yet." : "Edit receipt: Good - latest edit receipt exists.",
            latestValidation?.Succeeded == true ? "Validation receipt: Good - latest validation passed." : "Validation receipt: Waiting - validation still needs to run.",
            gitStatus.Available ? $"Git confidence: {gitStatus.Summary}." : "Git confidence: Review - status unavailable."
        ];
    }

    private static int BuildPatchConfidenceScore(
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        BehaviorTestPatchReadiness testPreview,
        CodingReceipt? latestValidation,
    ...
    {
        var score = 45;
        score += context.RankedTargets.Count > 0 ? 12 : 0;
        score += context.ImpactScore.Score >= 70 ? 8 : context.ImpactScore.Score >= 50 ? 4 : 0;
        score += synthesis.Candidates.Count > 0 ? 10 : 0;
        score += synthesis.PreviewReady ? 14 : 0;
        score += testPreview.Ready ? 10 : 0;
        score += latestEdit is not null ? 3 : 0;
        score += latestValidation?.Succeeded == true ? 8 : latestValidation?.Succeeded == false ? -8 : 0;
        score += gitStatus.Available ? 4 : 0;
        score -= context.RiskLabels.Any(label => label.Contains("generated", StringComparison.OrdinalIgnoreCase)
            || label.Contains("designer", StringComparison.OrdinalIgnoreCase)
            || label.Contains("high", StringComparison.OrdinalIgnoreCase)) ? 8 : 0;
        return Math.Clamp(score, 0, 100);
    }

    private static IReadOnlyList<string> BuildPatchConfidenceRiskRows(
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        BehaviorTestPatchReadiness testPreview,
        CodingReceipt? latestValidation,
        GitWorkingTreeStatus gitStatus)
    {
        return
        [
            context.RiskLabels.Count == 0 ? "File risk: Good - no special risk label detected." : $"File risk: Review - {FormatInlineList(context.RiskLabels)}.",
            synthesis.PreviewReady ? "Exactness gate: Good - old/new text is present." : "Exactness gate: Review - do not apply until patch text is exact.",
            testPreview.Ready ? "Proof gate: Good - targeted behavior route exists." : "Proof gate: Review - add/resolve behavior proof before closeout.",
            latestValidation?.Succeeded == false ? "Failure gate: Bad - repair latest failed validation first." : "Failure gate: Good - no failed validation receipt is blocking.",
            gitStatus.HasUncommittedChanges ? "Working tree: Review - inspect existing changes before applying more." : "Working tree: Good - no uncommitted change reported."
        ];
    }

    private static IReadOnlyList<string> BuildSliceExecutorPreviewRows(
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        BehaviorTestPatchReadiness testPreview)
    {
        return
        [
            context.RankedTargets.Count == 0 ? "Slice 1 target: Review - resolve file target." : $"Slice 1 target: {context.RankedTargets[0].RelativePath}.",
            testPreview.Ready ? $"Slice 2 test: {testPreview.Command}." : $"Slice 2 test: Review - {testPreview.Status}",
            synthesis.PreviewReady ? $"Slice 3 code: {synthesis.PreviewEdits.Count} exact edit(s)." : $"Slice 3 code: Review - {synthesis.Status}",
            "Slice 4 review: inspect semantic diff and changed files.",
            "Slice 5 validation: run targeted validation, then build/tests if risk requires it."
        ];
    }

    private static IReadOnlyList<string> BuildSliceApplyBoundaryRows(
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis)
    {
        return
        [
            $"Max edit count: {MaxPatchBundleEdits} exact file edit(s) per preview.",
            context.RiskLabels.Any(label => label.Contains("generated", StringComparison.OrdinalIgnoreCase))
                ? "Generated files: Hold - owner must explicitly approve."
                : "Generated files: Good - none selected by current target map.",
            synthesis.PreviewReady ? "Apply boundary: preview can be shown, then owner decides." : "Apply boundary: no apply until exact old/new text exists.",
            "Stop boundary: failed validation routes to failure-to-patch v3 before more edits."
        ];
    }

    private static IReadOnlyList<string> BuildFailureToPatchStateRows(
        CodingReceipt? latestValidation,
        GitWorkingTreeStatus gitStatus,
        bool hasPendingPreview)
    {
        return
        [
            hasPendingPreview ? "Pending preview: Review - apply or discard before repair." : "Pending preview: Good - no preview is blocking repair.",
            latestValidation is null ? "Latest validation: Waiting - no validation receipt found." : FormatReceiptSummary("Latest validation", latestValidation),
            latestValidation?.Succeeded == false ? "Failure status: Bad - route the first useful diagnostic." : "Failure status: Good - no failed validation receipt is active.",
            gitStatus.HasUncommittedChanges ? $"Working tree: Review - {gitStatus.Summary}." : "Working tree: Good - no uncommitted change reported."
        ];
    }

    private static IReadOnlyList<string> BuildFailureToPatchRouteRows(
        FeatureWorkContext context,
        CodingReceipt? latestValidation)
    {
        if (latestValidation is { Succeeded: false })
        {
            return
            [
                "Route: map the first compiler/build/test diagnostic.",
                $"Target focus: {FormatInlineList(context.RelativeCandidateFiles.Take(5))}",
                $"Repair preview: validation repair runner {context.Goal}",
                "Close loop: rerun the smallest validation command after repair."
            ];
        }

        return
        [
            "Route: no active failure; use validation chain planner before patching more.",
            $"Target focus: {FormatInlineList(context.RelativeCandidateFiles.Take(5))}",
            $"Next proof: {context.TestTarget.Command ?? "resolve targeted validation"}"
        ];
    }

    private static IReadOnlyList<string> BuildSemanticChangeReceiptRows(
        IReadOnlyList<string> changedFiles,
        CodingReceipt? latestValidation,
        CodingReceipt? latestEdit,
        CodingReceipt? latestGit)
    {
        return
        [
            changedFiles.Count == 0 ? "Changed files: Waiting - no changed files detected." : $"Changed files: {changedFiles.Count}.",
            latestEdit is null ? "Latest edit: Waiting - no edit receipt found." : FormatReceiptSummary("Latest edit", latestEdit),
            latestValidation is null ? "Latest validation: Waiting - no validation receipt found." : FormatReceiptSummary("Latest validation", latestValidation),
            latestGit is null ? "Latest git receipt: Waiting - no git closeout receipt found." : FormatReceiptSummary("Latest git receipt", latestGit),
            latestValidation?.Succeeded == true ? "Closeout status: Good - validation receipt is green." : "Closeout status: Review - validation proof still needed."
        ];
    }

    private static IReadOnlyList<string> BuildValidationChainRows(
        FeatureWorkContext context,
        CodingReceipt? latestValidation,
        GitWorkingTreeStatus gitStatus)
    {
        return
        [
            "1. Inspect: semantic diff summary and patch confidence score.",
            string.IsNullOrWhiteSpace(context.TestTarget.Command)
                ? "2. Targeted test: Review - resolve test target first."
                : $"2. Targeted test: {context.TestTarget.Command}.",
            "3. Build: confirm dotnet build after patch preview/apply.",
            "4. Tests: confirm dotnet test when source or shared behavior changed.",
            latestValidation?.Succeeded == true ? "5. Receipt: Good - latest validation passed." : "5. Receipt: Waiting - create validation receipt.",
            gitStatus.HasUncommittedChanges ? "6. Closeout: review changes before commit." : "6. Closeout: no uncommitted changes reported."
        ];
    }

    private static IReadOnlyList<string> BuildValidationCommandRows(
        FeatureWorkContext? context,
        CodingReceipt? latestValidation)
    {
        var targeted = context?.TestTarget.Command;
        return
        [
            string.IsNullOrWhiteSpace(targeted) ? "Targeted validation: resolve test target <goal>." : $"Targeted validation: {targeted}.",
            "Build command: confirm dotnet build.",
    ...
    private static IReadOnlyList<string> BuildBehaviorContractRows(FeatureWorkContext context)
    {
        ...
        return
        [
            $"Input: owner asks for {context.Goal}.",
            $"User action: use the feature lane, dashboard button, or owner phrase tied to {context.Goal}.",
            "Expected result: visible behavior or diagnostic output changes in the requested workflow.",
            "Failure behavior: explain what failed, preserve files, and show the next safe diagnostic.",
            topTarget is null ? "Primary target: not resolved yet." : $"Primary target: {topTarget.RelativePath} ({topTarget.Confidence}).",
        ...
        GitWorkingTreeStatus gitStatus)
    {
        if (latestValidation is { Succeeded: false })
        {
            return "Repair validation failure";
        }
        ...
        {
            return gitStatus.HasUncommittedChanges ? "Ready for safe commit check" : "Ready for completion receipt";
        }

        if (latestEdit is not null || context.ChangedFiles.Count > 0)
        {
            return "Validate changed files";
        }

        if (synthesis.PreviewReady)
        {
            return "Ready for patch preview";
        }

        if (context.RankedTargets.Count == 0)
        {
            return "Resolve edit target";
        }

        if (string.IsNullOrWhiteSpace(context.TestTarget.Command))
        {
            return "Resolve validation target";
        }

        return "Draft exact patch text";
    }

    private int BuildFeatureSessionScore(
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        CodingReceipt? latestValidation,
        CodingReceipt? latestEdit,
        GitWorkingTreeStatus gitStatus)
    {
        ...
        score += context.RankedTargets.Count > 0 ? 20 : 0;
        ...
        score += _lastPatchPreviewRequest is not null ? 10 : 0;
        score += latestEdit is not null || context.ChangedFiles.Count > 0 ? 10 : 0;
        score += latestValidation?.Succeeded == true ? 15 : latestValidation?.Succeeded == false ? -10 : 0;
        score += gitStatus.Available ? 5 : 0;
        score += gitStatus.Clean && latestValidation?.Succeeded == true ? 5 : 0;
        return Math.Clamp(score, 0, 100);
    }

    private IReadOnlyList<string> BuildFeatureSessionChecklistRows(
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        CodingReceipt? latestValidation,
        CodingReceipt? latestEdit,
        GitWorkingTreeStatus gitStatus)
    {
        var rows = new List<string>
        {
            context.Goal.Equals("current feature", StringComparison.OrdinalIgnoreCase)
                ? "Intent: Review - no specific feature goal was provided."
                : $"Intent: Good - {context.Goal}.",
            context.RankedTargets.Count == 0
                ? "Target: Review - no primary edit file is resolved."
                : $"Target: Good - {context.RankedTargets.First().RelativePath} ({context.RankedTargets.First().Confidence}).",
            string.IsNullOrWhiteSpace(context.TestTarget.Command)
                ? "Validation target: Review - no targeted command is resolved."
                : $"Validation target: Good - {context.TestTarget.Command}.",
            synthesis.PreviewReady
                ? $"Synthesis: Good - {synthesis.PreviewEdits.Count} preview-ready edit(s)."
                : synthesis.Candidates.Count > 0
                    ? "Synthesis: Review - exact anchors exist but new implementation text is still needed."
                    : "Synthesis: Waiting - no exact anchor or edit block yet.",
            _lastPatchPreviewRequest is not null
                ? $"Preview: Good - pending {_lastPatchPreviewRequest.Action}."
                : synthesis.PreviewReady
                    ? "Preview: Ready - run preview synthesized feature patch."
                    : "Preview: Waiting - no preview-ready patch yet.",
            latestEdit is not null || context.ChangedFiles.Count > 0
                ? "Apply: Review - changes exist and need validation."
                : _lastPatchPreviewRequest is not null
                    ? "Apply: Waiting - owner confirmation is required."
                    : "Apply: Waiting - preview must come first.",
            latestValidation?.Succeeded == true
                ? "Validation: Good - latest build/test receipt passed."
                : latestValidation?.Succeeded == false
                    ? "Validation: Bad - repair the latest failed build/test receipt."
                    : "Validation: Waiting - run the targeted validation after apply.",
            latestValidation?.Succeeded == true && !gitStatus.HasUncommittedChanges
                ? "Closeout: Good - no uncommitted code changes detected."
                : latestValidation?.Succeeded == true
                    ? "Closeout: Ready - run safe commit check."
                    : "Closeout: Waiting - validation must pass first."
        };
        return rows;
    }

    private IReadOnlyList<string> BuildFeatureSessionEvidenceRows(
        FeatureWorkContext context,
        CodingReceipt? latestValidation,
        CodingReceipt? latestEdit,
        CodingReceipt? latestGit,
        GitWorkingTreeStatus gitStatus)
    {
        var changed = context.ChangedFiles.Count == 0
            ? "none"
            : FormatInlineList(context.ChangedFiles.Take(6));
        return
        [
            $"Top target: {context.RankedTargets.FirstOrDefault()?.RelativePath ?? "none resolved yet"}.",
            _lastPatchPreviewRequest is null ? "Pending preview: none." : $"Pending preview: {_lastPatchPreviewRequest.Action}.",
            latestEdit is null ? "Latest edit: none." : FormatReceiptSummary("Latest edit", latestEdit),
            latestValidation is null ? "Latest validation: none." : FormatReceiptSummary("Latest validation", latestValidation),
            latestGit is null ? "Latest git check: none." : FormatReceiptSummary("Latest git check", latestGit),
    ...
    private string BuildFeatureSessionNextCommand(
        string goal,
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        CodingReceipt? latestValidation,
        CodingReceipt? latestEdit)
    {
        ...
        if (latestEdit is not null || context.ChangedFiles.Count > 0)
        {
            return latestValidation?.Succeeded == true
    ...
    private static bool IsFeatureEditReceipt(CodingReceipt receipt) =>
        receipt.Action is nameof(CodingToolAction.ApplyLastPatchPreview)
            or nameof(CodingToolAction.ReplaceText)
            or nameof(CodingToolAction.CreateFile)
            or nameof(CodingToolAction.AppendFile);

    private static bool IsFeatureGitReceipt(CodingReceipt receipt) =>
        receipt.Action.StartsWith("Git", StringComparison.OrdinalIgnoreCase)
        || receipt.Action is nameof(CodingToolAction.GitStatus)
            or nameof(CodingToolAction.GitDiff)
    ...
    private string ClassifyFeatureRunControllerPhase(
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        CodingReceipt? latestValidation,
        CodingReceipt? latestEdit,
        GitWorkingTreeStatus gitStatus)
    {
        ...
        if ((latestEdit is not null || context.ChangedFiles.Count > 0) && latestValidation?.Succeeded == true)
        {
            return gitStatus.HasUncommittedChanges ? "Closeout review" : "Completion receipt";
        }

        if (synthesis.PreviewReady)
        {
            return "Preview synthesized patch";
        }
        ...
        return "Draft exact patch";
    }

    private string BuildFeatureRunGoNoGo(
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        CodingReceipt? latestValidation,
        CodingReceipt? latestEdit,
        GitWorkingTreeStatus gitStatus)
    {
        if (_lastDotNetResult is { Succeeded: false } || latestValidation is { Succeeded: false })
        {
            return "No-go - repair the failed validation first.";
        }

        if (_lastPatchPreviewRequest is not null)
        {
            return "Hold - owner must review and approve the pending preview.";
        }

        if ((latestEdit is not null || context.ChangedFiles.Count > 0) && latestValidation?.Succeeded != true)
        {
            return "No-go - changed files need validation.";
        }

        if ((latestEdit is not null || context.ChangedFiles.Count > 0) && latestValidation?.Succeeded == true)
        {
            return gitStatus.HasUncommittedChanges ? "Go - ready for review and safe commit check." : "Go - ready for completion receipt.";
        }

        if (synthesis.PreviewReady)
        {
            return "Go - preview-only patch is ready.";
        }

        if (context.RankedTargets.Count == 0 || string.IsNullOrWhiteSpace(context.TestTarget.Command))
        {
            return "Hold - resolve target and validation route first.";
        }

        return "Go - draft the exact patch block.";
    }

    private int BuildFeatureRunControllerScore(
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        CodingReceipt? latestValidation,
        CodingReceipt? latestEdit,
        GitWorkingTreeStatus gitStatus)
    {
        ...
        score += string.IsNullOrWhiteSpace(context.TestTarget.Command) ? 0 : 15;
        score += synthesis.PreviewReady ? 15 : synthesis.Candidates.Count > 0 ? 8 : 0;
        score += _lastPatchPreviewRequest is not null ? 10 : 0;
        score += latestEdit is not null || context.ChangedFiles.Count > 0 ? 10 : 0;
        score += latestValidation?.Succeeded == true ? 20 : latestValidation?.Succeeded == false ? -15 : 0;
        score += gitStatus.Available ? 5 : 0;
        score += gitStatus.Clean && latestValidation?.Succeeded == true ? 10 : 0;
        return Math.Clamp(score, 0, 100);
    }

    private IReadOnlyList<string> BuildFeatureRunStateRows(
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        CodingReceipt? latestValidation,
        CodingReceipt? latestEdit)
    {
        ...
        [
            _lastPatchPreviewRequest is null
                ? synthesis.PreviewReady ? "Preview: Ready - synthesized preview can be shown." : "Preview: Waiting - no pending preview."
                : $"Preview: Pending - {_lastPatchPreviewRequest.Action}.",
            latestEdit is null && context.ChangedFiles.Count == 0
                ? "Apply: Waiting - no applied edit or changed file evidence."
                : $"Apply: Review - {Math.Max(context.ChangedFiles.Count, latestEdit is null ? 0 : 1)} edit signal(s) present.",
            latestValidation?.Succeeded == true
                ? "Validation: Good - latest build/test receipt passed."
                : latestValidation?.Succeeded == false
                    ? "Validation: Bad - repair runner should go next."
                    : "Validation: Waiting - no build/test receipt for this run.",
            _lastDotNetResult is { Succeeded: false }
                ? "Repair: Active - failed validation is loaded."
                : "Repair: Clear - no active failed validation loaded.",
            context.RankedTargets.Count == 0
                ? "Target: Review - no ranked edit target."
                : $"Target: Good - {context.RankedTargets.First().RelativePath} ({context.RankedTargets.First().Confidence}).",
            string.IsNullOrWhiteSpace(context.TestTarget.Command)
                ? "Validation route: Review - no targeted command resolved."
                : $"Validation route: Good - {context.TestTarget.Command}."
        ];
    }

    private IReadOnlyList<string> BuildFeatureRunStopGateRows(
        FeatureWorkContext context,
        CodingReceipt? latestValidation,
        GitWorkingTreeStatus gitStatus)
    {
        ...
        if (_lastPatchPreviewRequest is not null)
        {
            rows.Add("Pending preview must be reviewed, applied, or discarded before validation/commit.");
        }

        if (_lastDotNetResult is { Succeeded: false } || latestValidation is { Succeeded: false })
        {
            rows.Add("Latest validation is failed; run repair before closeout.");
        }

        if (context.ChangedFiles.Count > 0 && latestValidation?.Succeeded != true)
        {
            rows.Add("Changed files exist without passing validation.");
        }

        if (string.IsNullOrWhiteSpace(context.TestTarget.Command))
        {
            rows.Add("Targeted validation command is unresolved.");
        }

        if (context.RiskLabels.Any(label => label.StartsWith("Protected", StringComparison.OrdinalIgnoreCase)
            || label.StartsWith("High", StringComparison.OrdinalIgnoreCase)))
        {
            rows.Add("High-risk/protected file signal requires explicit owner review.");
        }

        if (!gitStatus.Available)
        {
            rows.Add("Git status is unavailable.");
        }

        return rows.Count == 0 ? ["None - controller has no active stop gate."] : rows;
    }

    private string BuildFeatureRunControllerNextCommand(
        string goal,
        FeatureWorkContext context,
        FeaturePatchSynthesis synthesis,
        CodingReceipt? latestValidation,
    ...
    private static IReadOnlyList<string> BuildPatchSliceRows(FeatureWorkContext context)
    {
        if (context.RankedTargets.Count == 0)
        {
            return
            [
                "Discovery slice - inspect likely files first; reason no ranked target is available; risk review; validation resolve test target before preview."
            ];
        }

        var validation = context.ValidationRows.FirstOrDefault(row => row.Contains("Targeted test", StringComparison.OrdinalIgnoreCase))
            ?? context.ValidationRows.FirstOrDefault()
            ?? "resolve a validation command first";
        return context.RankedTargets
            .Take(3)
            .Select((target, index) =>
                $"Patch slice {index + 1} - {target.RelativePath}: {string.Join("; ", target.Reasons.Take(2))}; risk {target.Risk}; expected behavior supports {context.Goal}; validation {validation}")
            .ToList();
    }

    private static IReadOnlyList<string> BuildTestStubRows(FeatureWorkContext context)
    {
        var firstTarget = context.RankedTargets.FirstOrDefault();
        var targetName = firstTarget is null
            ? "CurrentFeature"
            : Path.GetFileNameWithoutExtension(firstTarget.RelativePath).Replace(".", string.Empty, StringComparison.OrdinalIgnoreCase);
        return
        [
            $"Parser route test: {targetName}_routes_owner_phrase_to_expected_action.",
            $"Service output test: {targetName}_returns_heading_status_and_no_file_change_boundary.",
            $"Behavior regression test: {targetName}_preserves_expected_result_for_{NormalizeTestNameToken(context.Goal)}.",
    ...
    {
        var topTarget = context.RankedTargets.FirstOrDefault();
        var protectedFiles = context.RelativeCandidateFiles
            .Where(file => ClassifyFileRisk(file).StartsWith("Protected", StringComparison.OrdinalIgnoreCase))
            .Take(5)
            .ToList();
        var targetReady = topTarget is not null && !topTarget.Confidence.Equals("Low", StringComparison.OrdinalIgnoreCase);
        var testReady = !string.IsNullOrWhiteSpace(context.TestTarget.Command);
        var protectedReady = protectedFiles.Count == 0;
        var ready = targetReady && testReady && protectedReady;
        return
        [
            targetReady ? $"Target confidence: Good - {topTarget!.RelativePath} ({topTarget.Confidence})." : "Target confidence: Review - no medium/high target yet.",
            "Behavior contract: Good - input, action, result, failure behavior, and permission boundary are defined.",
            testReady ? $"Test target: Good - {context.TestTarget.Command}." : "Test target: Review - resolve a targeted test before behavior-changing apply.",
    ...
    private static IReadOnlyList<string> BuildPostPatchValidationRows(FeatureWorkContext context)
    {
        var rows = new List<string>();
        rows.Add(string.IsNullOrWhiteSpace(context.TestTarget.Command)
            ? "1. Targeted test: resolve test target before running validation."
            : $"1. Targeted test: {context.TestTarget.Command}");
        rows.AddRange(context.ValidationRows
            .Where(row => !row.Contains("Targeted test", StringComparison.OrdinalIgnoreCase))
            .Take(4)
            .Select((row, index) => $"{index + 2}. {row}"));
        rows.Add("Final: review current changes, then can i safely commit.");
        return rows;
    }

    private static string NormalizeTestNameToken(string value)
    {
    ...
    private async Task<CodingToolResult> ShowStopConditionDetectorAsync(
        CancellationToken cancellationToken,
    ...
    private async Task<CodingToolResult> ShowFeatureCompletionReceiptAsync(
        CancellationToken cancellationToken,
        FeatureWorkContext? context = null)
    {
        context ??= await BuildFeatureWorkContextAsync(new CodingToolRequest(CodingToolAction.ShowFeatureCompletionReceipt, null, Query: "current feature"), cancellationToken).ConfigureAwait(false);
        var changedFiles = context.ChangedFiles;
        var receipts = ReadRecentReceipts(MaxReceiptEntries);
        var latestValidation = receipts.LastOrDefault(IsDotNetReceipt);
        var latestGit = receipts.LastOrDefault(receipt => receipt.Action.StartsWith("Git", StringComparison.OrdinalIgnoreCase)
            || receipt.Action is nameof(CodingToolAction.GitStatus) or nameof(CodingToolAction.GitDiff) or nameof(CodingToolAction.ReviewCurrentChanges));
        ...
        var lines = new List<string>
        {
            ...
            "- Behavior contract and patch slices are reviewed.",
        ...
        FeatureWorkContext? context = null)
    {
        ...
        lines.AddRange(context.RankedTargets.Count == 0 ? ["- none resolved yet"] : context.RankedTargets.Take(6).Select(FormatEditTargetCandidate));
        lines.Add("Behavior contract:");
        lines.AddRange(BuildBehaviorContractRows(context).Take(4).Select(row => $"- {row}"));
        lines.Add("Patch slices:");
        lines.AddRange(BuildPatchSliceRows(context).Take(3).Select(row => $"- {row}"));
        lines.Add("Prioritized tests:");
        lines.AddRange(context.TestTarget.PrioritizedTests.Count == 0 ? ["- none resolved yet"] : context.TestTarget.PrioritizedTests.Take(6).Select(row => $"- {row}"));
        lines.Add("Validation queue:");
        lines.AddRange(context.ValidationRows.Select(row => $"- {row}"));
        AddReferenceGraphLines(lines, context.Graph, includeSymbols: true);
        AddAutonomousPreflightLines(lines, context.RelativeCandidateFiles, context.Graph, context.TestTarget, _lastPatchPreviewRequest is not null);
        lines.Add("Apply gate:");
        lines.AddRange(BuildApplyGateRows(context).Take(5).Select(row => $"- {row}"));
        lines.Add("Stop gates:");
    ...
    private async Task<CodingToolResult> ShowBuildFeatureLaneAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        var context = await BuildFeatureWorkContextAsync(request, cancellationToken).ConfigureAwait(false);
        var goal = context.Goal;
        var intent = BuildFeatureIntentPacket(new CodingToolRequest(CodingToolAction.BuildFeatureIntentPacket, null, Query: goal), context);
        var tests = await PlanBehaviorTestsAsync(new CodingToolRequest(CodingToolAction.PlanBehaviorTests, null, Query: goal), cancellationToken, context).ConfigureAwait(false);
        var slices = await PlanImplementationSlicesAsync(new CodingToolRequest(CodingToolAction.PlanImplementationSlices, null, Query: goal), cancellationToken, context).ConfigureAwait(false);
        var execution = await ShowFeatureExecutionPacketAsync(new CodingToolRequest(CodingToolAction.ShowFeatureExecutionPacket, null, Query: goal), cancellationToken, context).ConfigureAwait(false);
        var stop = await ShowStopConditionDetectorAsync(cancellationToken, context.ChangedFiles).ConfigureAwait(false);
        var receipt = await ShowFeatureCompletionReceiptAsync(cancellationToken, context).ConfigureAwait(false);
        var lines = new List<string>
        {
            "Build Feature lane:",
            "No files were changed.",
    ...
    private async Task<CodingToolResult> PlanSemanticEditAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        ...
        var rankedTargets = RankEditTargetCandidates(goalTerms, hits, candidateFiles, changedFiles);
        var impactScore = BuildEditImpactScore(rankedTargets, hits, candidateFiles, null);
        var lines = new List<string>
        {
            "Semantic edit plan:",
            "No files were changed.",
            ...
            $"Edit impact score: {impactScore.Score}/100 ({impactScore.Confidence})",
            $"Edit impact reasons: {FormatInlineList(impactScore.Reasons)}",
            "Ranked edit targets:"
        };
    ...
    private async Task<CodingToolResult> PlanSafeEditWorkflowAsync(CodingToolRequest request, CancellationToken cancellationToken)
    {
        ...
        lines.Add("Autonomous validation plan:");
        lines.AddRange(BuildAutonomousValidationPlanRows(goal, GetProjectIndexStatus(), testTarget).Select(row => $"- {row}"));
        lines.Add("Edit target validation:");
        lines.Add(candidateFiles.Count == 0
    ...
    private void PersistRouteDiffTracker(
        ...
        IReadOnlyList<string> routeRepairPackets)
    {
    ...
    private CodingToolResult ShowCommandSurfaceDoctor()
    {
        ...
        var routeRepairPackets = BuildRouteRepairPacketRows(missingService, missingPolicy, missingParserOrTests, missingDashboardTargets);
        PersistRouteDiffTracker(actions.Length, missingService, missingPolicy, missingParserOrTests, missingDashboardTargets, routeDriftRows, routeRepairPackets);
        var lines = new List<string>
        {
            ...
            $"- Parser/test coverage: {(missingParserOrTests.Count == 0 ? "Good" : "Review")}",
            $"- Dashboard bindings: {(missingDashboardTargets.Count == 0 ? "Good" : "Bad")}",
            "Service gaps:"
        };
        lines.InsertRange(lines.IndexOf("Route repair packets:"), routeDriftRows.Select(row => $"- {row}"));
        lines.InsertRange(lines.IndexOf("Dashboard summary rows:"), routeRepairPackets.Select(row => $"- {row}"));
    ...
    private async Task<CodingToolResult> ReviewCurrentChangesAsync(CancellationToken cancellationToken)
    {
        ...
        var testFiles = changedFiles
            ...
            .ToList();
        var projectImpact = BuildProjectImpact(changedFiles, GetWorkspaceProjectSummaries());
        var changedAreas = changedFiles.Select(ClassifyChangedArea).Distinct(StringComparer.OrdinalIgnoreCase).Take(6).ToList();
        var riskLabels = changedFiles.Select(ClassifyFileRisk).Distinct(StringComparer.OrdinalIgnoreCase).Take(6).ToList();
        ...
        var lines = new List<string>
        {
            ...
            $"Affected projects: {FormatInlineList(projectImpact.AffectedProjects)}",
            $"Build order slice: {FormatInlineList(projectImpact.BuildOrderProjects)}",
            $"Change areas: {FormatInlineList(changedAreas)}",
            $"Risk labels: {FormatInlineList(riskLabels)}"
        };
        ...
        string output)
    {
        ...
        var diagnostics = ExtractStructuredDiagnostics(action, output, MaxDiagnosticLines);
        ...
        lines.AddRange(BuildFailureTriageLines(action, output));
        lines.Add("Structured diagnostics:");
        lines.AddRange(diagnostics.Select(diagnostic => $"- {diagnostic}"));
        return string.Join(Environment.NewLine, lines);
    }

    private static IReadOnlyList<string> ExtractStructuredDiagnostics(CodingToolAction action, string output, int limit)
    {
    ...
    private static IReadOnlyList<string> BuildFailureTriageLines(CodingToolAction action, string output)
    {
        var diagnostics = ExtractStructuredDiagnostics(action, output, 40);
        ...
        var lines = new List<string>
        {
            $"Failure type: {ClassifyFailureMessage(output)}",
            codes.Count == 0 ? "Repeated codes: none detected" : $"Repeated codes: {string.Join(", ", codes)}",
            "First useful errors:"
        };
        ...
        lines.Add(action == CodingToolAction.Test
            ...
            : "- Fix the first root error before chasing later duplicate errors.");
        return lines;
    }
    ...
    private static string? ExtractDiagnosticCode(string line)
    {
        ...
        return tokens.FirstOrDefault(token => token.Length >= 4
            && (token.StartsWith("CS", StringComparison.OrdinalIgnoreCase)
                || token.StartsWith("NU", StringComparison.OrdinalIgnoreCase)
                || token.StartsWith("MSB", StringComparison.OrdinalIgnoreCase)
                || token.StartsWith("NETSDK", StringComparison.OrdinalIgnoreCase)));
    }
    ...
    private sealed record FeatureWorkContext(
        ...
        IReadOnlyList<SemanticSymbolHit> SemanticHits,
        IReadOnlyList<string> CandidateFiles,
        IReadOnlyList<string> RelativeCandidateFiles,
        IReadOnlyList<EditTargetCandidate> RankedTargets,
        TestTargetRecommendation TestTarget,
        EditImpactScore ImpactScore,
        IReadOnlyList<string> RiskLabels,
        IReadOnlyList<string> ValidationRows);

    private sealed record EditImpactScore(
        int Score,
        string Confidence,
    ...
    private sealed record ProjectIndexAwareness(
        ...
        IReadOnlyList<string> ProjectIntent,
        IReadOnlyList<string> CompressedRepoMap);
    ...
    private sealed record ProjectIndexSnapshot(
        ...
        IReadOnlyList<string> ProjectIntent,
        IReadOnlyList<string> CompressedRepoMap,
        IReadOnlyList<string> SymbolStalenessRows,
        IReadOnlyList<string> RefreshAutomationRows);
