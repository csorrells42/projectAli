    ...
    string? pdfWorkspaceRoot = null) : ILocalCodingTool
{
    ...
    private async Task<CodingToolResult> ShowMiniCodexStatusAsync(CancellationToken cancellationToken)
    {
        ...
        var scores = new (string Name, int Score, string Note)[]
        {
            ...
            ("Dashboard usability", 98, "chat-first programming workspace, current project path picker, new-project button, working-notes activity pane, shared chat composer, cancel/next steering controls, command queue rows, status-only rows, route diff repair packets, and one-click score v3")
        };
        var overall = 99;
        var lines = new List<string>
        {
