...
internal sealed class AliToolCatalog
{
    ...
    internal static string BuildInstructions(
        ...
        string.Join(
            ...
            "For a new Arduino sketch that the user asks you to create and compile, call arduino_create_and_compile with the complete source, an approved virtual .ino path whose filename matches its parent folder, and the explicit board FQBN. This one operation creates the folder and file, invokes the real compiler, and returns firmware artifacts. Never claim Arduino compilation is unavailable when this registered tool exists.",
