...
internal sealed class AliToolCatalog
{
    ...
    internal static string BuildInstructions(
        ...
        string.Join(
            ...
            "The list_available_tools result is a compact authoritative collection with Total, Schema, Sources, and Tools. For a requested complete inventory, emit exactly Total tool rows from Tools in returned order, resolve sourceId through Sources, and never invent, duplicate, extrapolate, or omit a tool. Use the descriptions already attached to your registered tool schemas when the user asks for descriptions; do not call list_available_tools a second time merely because its compact rows omit repeated descriptions.",
