    ...
    Func<CoordinatorTurnContext?> turnAccessor) : IChatClient
{
    ...
    private static string BuildDecisionInstruction(IReadOnlyList<AIFunctionDeclaration> tools)
    {
        ...
        return string.Join(
            ...
            "When registered tools can fulfill the newest request, use them instead of claiming incapability or giving manual shell instructions. For a new C# application, create the project, replace the template with the complete requested source, use Roslyn to inspect uncertainty, build it through MSBuild, fix errors, and run it only when explicitly requested. Never treat an untouched project template as the requested application.",
