...
static async Task TestLocalCodingToolSynthesizesWpfCounterStarter()
{
    ...
    await AssertWpfStarterAsync(
        ...
        ["ResourceDictionary Source=\"AliDashboardStyles.xaml\"", "BooleanToVisibilityConverter", "DashboardStatusBrushConverter", "Window.InputBindings", "KeyBinding Key=\"F5\"", "KeyBinding Key=\"Escape\"", "DashboardDetailTemplate", "DashboardDetailCard", "DataType=\"{x:Type local:OverviewDashboardViewModel}\"", "DataType=\"{x:Type local:ActivityDashboardViewModel}\"", "DataType=\"{x:Type local:SettingsDashboardViewModel}\"", "Content=\"{Binding SelectedWorkspaceView}\"", "ContentControl", "SelectedItem=\"{Binding SelectedItem, Mode=TwoWay}\"", "DashboardHeaderCardStyle", "NavigationTreeView", "ItemsSource=\"{Binding NavigationItems}\"", "HierarchicalDataTemplate", "IsExpanded\" Value=\"{Binding IsExpanded, Mode=TwoWay}\"", "IsSelected\" Value=\"{Binding IsSelected, Mode=TwoWay}\"", "SelectedNavigationSummary", "Text=\"{Binding SearchText, UpdateSourceTrigger=PropertyChanged}\"", "ItemsSource=\"{Binding ItemsView}\"", "DataGrid.GroupStyle", "GroupStyle.HeaderTemplate", "DataGridTemplateColumn", "DataGridTemplateColumn.CellTemplate", "Binding Path=\"Status\" Converter=\"{StaticResource DashboardStatusBrushConverter}\"", "CornerRadius=\"8\"", "ProgressBar", "IsIndeterminate=\"{Binding IsBusy}\"", "Command=\"{Binding CancelRefreshCommand}\"", "Command=\"{Binding RemoveSelectedItemCommand}\"", "ValidatesOnNotifyDataErrors=True", "DashboardInputTextBoxStyle", "NewItemError", "GridSplitter", "StatusBar", "Command=\"{Binding AddItemCommand}\""],
