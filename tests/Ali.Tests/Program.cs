...
static Task TestCodingParserRoutesAdvancedCodingHelpers()
{
    ...
    Equal(CodingToolAction.ShowFeatureSessionLedger, sessionLedgerRequest.Action);
    Equal("Save button", sessionLedgerRequest.Query);
...
static async Task TestLocalCodingToolShowsCodingReadinessHelpers()
{
    ...
    Contains("Feature session ledger v1", sessionLedger.Message);
    Contains("Current stage:", sessionLedger.Message);
    Contains("Session score:", sessionLedger.Message);
...
static Task TestProgrammingDashboardExposesCockpitCommands()
{
    ...
    var codeBehindPath = Path.Combine(repository.FullName, "src", "Ali.App.Wpf", "ProgrammingDashboardWindow.xaml.cs");
    ...
    var searchIndex = 0;
    while ((searchIndex = dashboard.IndexOf("<Button", searchIndex, StringComparison.OrdinalIgnoreCase)) >= 0)
    {
        visibleButtonCount++;
        searchIndex += "<Button".Length;
    }

    Equal(true, visibleButtonCount <= 10);
    Equal(false, dashboard.Contains("UniformGrid Columns=\"1\"", StringComparison.OrdinalIgnoreCase));
    Equal(false, dashboard.Contains("Coding Cockpit", StringComparison.OrdinalIgnoreCase));
    Equal(false, dashboard.Contains("Build Loop", StringComparison.OrdinalIgnoreCase));
    Equal(false, dashboard.Contains("Patch And Handoff", StringComparison.OrdinalIgnoreCase));
    Equal(false, dashboard.Contains("Patch Synth v2", StringComparison.OrdinalIgnoreCase));
    Equal(false, dashboard.Contains("Evidence Pack", StringComparison.OrdinalIgnoreCase));
    Equal(false, dashboard.Contains("Author Flow", StringComparison.OrdinalIgnoreCase));
    Equal(false, dashboard.Contains("Completion Receipt", StringComparison.OrdinalIgnoreCase));

    Contains("CodingCurrentSolutionOrProjectPathText", dashboard);
    Contains("Select Project", dashboard);
    ...
    Contains("Cancel", dashboard);
    Contains("StopCommand", dashboard);
    Contains("Next", dashboard);
    Contains("SendProgrammingNextCommand", dashboard);
    Contains("MaintenanceReceiptPath", dashboard);

    Contains("owner.Hide()", codeBehind);
    Contains("_hiddenOwner.Show()", codeBehind);
...
static async Task TestLocalCodingToolSynthesizesWpfCounterStarter()
{
    ...
    Contains("xmlns:diag=\"clr-namespace:System.Diagnostics;assembly=WindowsBase\"", generatedDashboardXaml);
    ...
    Contains("Width=\"{Binding Data.NavigationColumnWidth, Source={StaticResource DashboardCommandsProxy}, Mode=TwoWay}\"", generatedDashboardXaml);
    Contains("Width=\"{Binding Data.DetailsColumnWidth, Source={StaticResource DashboardCommandsProxy}, Mode=TwoWay}\"", generatedDashboardXaml);
    Contains("Height=\"{Binding Data.OutputPanelHeight, Source={StaticResource DashboardCommandsProxy}, Mode=TwoWay}\"", generatedDashboardXaml);
    Contains("OutputPaneGroupBox", generatedDashboardXaml);
    Contains("Header=\"Output\"", generatedDashboardXaml);
    Contains("SelectedIndex=\"{Binding SelectedOutputPanelIndex, Mode=TwoWay}\"", generatedDashboardXaml);
    Contains("TabItem Header=\"Problems\"", generatedDashboardXaml);
    Contains("ItemsSource=\"{Binding Problems}\"", generatedDashboardXaml);
    Contains("DataGridTextColumn Header=\"Severity\"", generatedDashboardXaml);
    Contains("DataGridTextColumn Header=\"Source\"", generatedDashboardXaml);
    Contains("DataGridTextColumn Header=\"Message\"", generatedDashboardXaml);
    Contains("diag:PresentationTraceSources.TraceLevel=High", generatedDashboardXaml);
    Contains("NotifyOnValidationError=True", generatedDashboardXaml);
    ...
    Contains("IDashboardThemeService", generatedDashboardViewModel);
    Contains("NullDashboardThemeService", generatedDashboardViewModel);
    ...
    Contains("GridLength _outputPanelHeight", generatedDashboardViewModel);
    Contains("NavigationColumnWidth", generatedDashboardViewModel);
    Contains("DetailsColumnWidth", generatedDashboardViewModel);
    Contains("OutputPanelHeight", generatedDashboardViewModel);
    Contains("SelectedOutputPanelIndex", generatedDashboardViewModel);
    Contains("ObservableCollection<DashboardProblem> Problems", generatedDashboardViewModel);
    Contains("public sealed record DashboardProblem", generatedDashboardViewModel);
    ...
    Contains("new GridLength(Math.Max(120d, state.OutputPanelHeight))", generatedDashboardViewModel);
    Contains("Math.Clamp(state.SelectedOutputPanelIndex, 0, 1)", generatedDashboardViewModel);
    ...
    Contains("DashboardSelectionBehavior", generatedDashboardViewModel);
    Contains("ScrollSelectedItemIntoViewProperty", generatedDashboardViewModel);
    Contains("WeakEventManager<Selector, SelectionChangedEventArgs>.AddHandler", generatedDashboardViewModel);
    Contains("WeakEventManager<Selector, SelectionChangedEventArgs>.RemoveHandler", generatedDashboardViewModel);
    Contains("ScrollCurrentSelectionIntoView", generatedDashboardViewModel);
    Contains("dataGrid.ScrollIntoView(item)", generatedDashboardViewModel);
    Contains("listBox.ScrollIntoView(item)", generatedDashboardViewModel);
    Contains("DispatcherPriority.Background", generatedDashboardViewModel);
    Contains("DashboardAdaptiveWrapPanel : Panel", generatedDashboardViewModel);
    Contains("MinItemWidthProperty", generatedDashboardViewModel);
    Contains("FrameworkPropertyMetadataOptions.AffectsMeasure", generatedDashboardViewModel);
    Contains("FrameworkPropertyMetadataOptions.AffectsArrange", generatedDashboardViewModel);
    Contains("MeasureOverride(Size availableSize)", generatedDashboardViewModel);
    Contains("ArrangeOverride(Size finalSize)", generatedDashboardViewModel);
    Contains("InternalChildren", generatedDashboardViewModel);
    ...
    var confirmDialogPath = Path.Combine(projectDirectory, "DashboardConfirmDialog.xaml");
    var confirmDialogCodeBehindPath = Path.Combine(projectDirectory, "DashboardConfirmDialog.xaml.cs");
    ...
    Equal(true, File.Exists(confirmDialogPath));
    Equal(true, File.Exists(confirmDialogCodeBehindPath));
    ...
    var confirmDialog = await File.ReadAllTextAsync(confirmDialogPath);
    var confirmDialogCodeBehind = await File.ReadAllTextAsync(confirmDialogCodeBehindPath);
    AssertXamlWellFormed(dashboardStyles);
    AssertXamlWellFormed(detailCard);
    AssertXamlWellFormed(confirmDialog);
    AssertCSharpSyntaxClean(detailCardCodeBehind, detailCardCodeBehindPath);
    AssertCSharpSyntaxClean(confirmDialogCodeBehind, confirmDialogCodeBehindPath);
    ...
    Contains("DashboardBusyOverlayStyle", dashboardStyles);
    Contains("DynamicResource DashboardHeaderBrush", dashboardStyles);
    Contains("DynamicResource DashboardHeaderForegroundBrush", dashboardStyles);
    Contains("DynamicResource DashboardSubtleForegroundBrush", dashboardStyles);
    Contains("DynamicResource DashboardAccentBrush", dashboardStyles);
    Contains("DynamicResource DashboardAccentHoverBrush", dashboardStyles);
    ...
    Contains("xmlns:diag=\"clr-namespace:System.Diagnostics;assembly=WindowsBase\"", detailCard);
    ...
    Contains("VisualState x:Name=\"ReviewState\"", detailCard);
    Contains("VisualState x:Name=\"DraftState\"", detailCard);
    Contains("ColorAnimation", detailCard);
    Contains("Storyboard.TargetName=\"StateAccentBorder\"", detailCard);
    Contains("Storyboard.TargetProperty=\"(Border.BorderBrush).(SolidColorBrush.Color)\"", detailCard);
    Contains("x:Name=\"StateAccentBorder\"", detailCard);
    Contains("DashboardDetailCardStyle", detailCard);
    Contains("Item.Name, ElementName=Root", detailCard);
    Contains("diag:PresentationTraceSources.TraceLevel=High", detailCard);
    ...
    Contains("new PropertyMetadata(null, OnItemChanged)", detailCardCodeBehind);
    Contains("Loaded += DashboardDetailCard_Loaded", detailCardCodeBehind);
    ...
    Contains("OnItemChanged", detailCardCodeBehind);
    Contains("DashboardDetailCard_Loaded", detailCardCodeBehind);
    Contains("UpdateVisualState", detailCardCodeBehind);
    Contains("VisualStateManager.GoToElementState", detailCardCodeBehind);
    Contains("StringComparison.OrdinalIgnoreCase", detailCardCodeBehind);
    Contains("MainWindowViewModel.DashboardItem? Item", detailCardCodeBehind);
    Contains("GetValue(ItemProperty)", detailCardCodeBehind);
    Contains("WindowStartupLocation=\"CenterOwner\"", confirmDialog);
    Contains("ShowInTaskbar=\"False\"", confirmDialog);
    Contains("ResourceDictionary Source=\"AliDashboardStyles.xaml\"", confirmDialog);
    Contains("Title=\"{Binding Title}\"", confirmDialog);
    Contains("Text=\"{Binding Message}\"", confirmDialog);
    Contains("IsCancel=\"True\"", confirmDialog);
    Contains("IsDefault=\"True\"", confirmDialog);
    Contains("DashboardPrimaryButtonStyle", confirmDialog);
    Contains("partial class DashboardConfirmDialog : Window", confirmDialogCodeBehind);
    Contains("DashboardConfirmDialogViewModel", confirmDialogCodeBehind);
    Contains("DataContext = new DashboardConfirmDialogViewModel(request);", confirmDialogCodeBehind);
    Contains("DialogResult = true", confirmDialogCodeBehind);
    Contains("PrimaryText = request.PrimaryText", confirmDialogCodeBehind);
    Contains("CancelText = request.CancelText", confirmDialogCodeBehind);
}
